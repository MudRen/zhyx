...
成套装备指令："HIG"huanxing"NOR","HIG"zuhe"NOR"
...
int do_zuhe()
{
	...
	me = this_player();
        ...
        head =new("clone/armor/tianxiang_head");    

        message_vision("$N把从怀中一套"HIC"[天祥防具]"NOR"合成了一件"HIG"天翔头盔"NOR"。\n", me);
        message("channel:chat", HIR"【传闻】"HIG + me->query("name")+"得到一件天翔头盔啦！\n"NOR,users() );        
	head->move(me);
	destruct(present("longfeng zhanjia",me));
	destruct(present("yunpan huyao",me));
	destruct(present("jinyun huxin",me));
	destruct(present("tianling huwan",me));
	destruct(present("jinjin pifeng",me));
