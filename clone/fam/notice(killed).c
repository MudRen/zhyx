...
void create()
{
        set_name(HIY "十大杯具人物排行榜" NOR, ({ "killed wall", "killed"}));
        set_weight(500000);
        if (clonep())
                set_default_object(__FILE__);
        else {
                set("long", HIY "\n这是一面由整块墨玉雕琢而成的十大杯具人物排行榜。上面"
                            "刻着当今武林中\n死得最多的十大杯具人物，你"
                            "可以通过(view)来查阅排名。\n" NOR);
...
int do_view(string arg)
{
        ...
        for (i = 0; i < 10; i++)
        {
                ...
                msg += sprintf(HIW "┃" HIG "  %-5s %-22s%-10s %5d  " HIW "┃\n" NOR,
                        ...
                        list[i]->query("family") ?
                        list[i]->query("family/family_name") : "一般不死",
                        get_score(list[i]));
        }
        msg += HIW "┗━━━━━━━━━━━━━━━━━━━━━━━━┛\n" NOR;
...
int get_score(object ob)
{
        ...
        score = (int)ob->query("combat/dietimes") / 1;
        return score;
}
