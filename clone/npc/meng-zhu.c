// meng-zhu.c 武林盟主
// colored the title of 武林盟主(player) ,   XgChen, 1/15/1997
#include <ansi.h>
...
int do_kill()
{
	...
	for(i=0; i<4; i++) {
		...
		else	this_object()->fight_ob(this_player());		
	}
	...
	//if ( me->query("winner") == ob->query("id") ) 
{
	...
	if (wizardp(this_player()))
        ...
        if (this_player()->query("weiwang")<10000)
		return notify_fail("你有这个威望做盟主么？\n");
...
int checking(object me, object ob)
{
	...
	if (( (int)me->query("qi")*100 / my_max_qi) <= 50 ) {
		...
		zhu=new("/clone/fam/max/naobaijin1");
		zhu->move(ob);
		command("tell "+ob->query("id")+" 恭喜！为了表扬你我将我珍藏的"HIR"元神紫金丹"HIG"给你!\n"NOR);
...
//full玩家 by xgchen
		this_player()->set("max_qi",    hp_status["max_qi"]);
		this_player()->set("eff_qi",    hp_status["max_qi"]);
		this_player()->set("qi",        hp_status["max_qi"]);
		this_player()->set("max_jing",  hp_status["max_jing"]);
		this_player()->set("eff_jing",  hp_status["max_jing"]);
		this_player()->set("jing",      hp_status["max_jing"]);
		this_player()->set("max_neili", hp_status["max_neili"]);
		this_player()->set("neili",     hp_status["max_neili"]);
		this_player()->set("jiali",     hp_status["jiali"]);
		this_player()->set("gender",    hp_status["gender"]);
		this_player()->set("combat_exp",hp_status["combat_exp"]);
		
		me->set("str", hp_status["str"]*3/2);
		me->set("int", hp_status["int"]*3/2);
		me->set("con", hp_status["con"]*3/2);
		me->set("dex", hp_status["dex"]*3/2);
		me->set("age", hp_status["age"]*3/2);
		...
		me->set("jiali",     hp_status["jiali"]*3/2);
		me->set("gender",    hp_status["gender"]);
		me->set("combat_exp",hp_status["combat_exp"]*10/2);
...
/* copy combat exp values */
	...
	ob->set("combat_exp", me->query("combat_exp")*10/2);
