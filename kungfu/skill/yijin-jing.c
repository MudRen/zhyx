...
int practice_skill(object me)
{
    ...
    if ((int)me->query("jingli") < 80)
        return notify_fail("你的体力不够修习易筋经。\n");
    if ((int)me->query("neili") < 50)
        return notify_fail("你的内力不够修习易筋经。\n");
...
{
    return (["status_msg":HIR + me->name() + "脸上红光隐现，气象庄严" NOR,
...
int ob_hit(object ob, object me, int damage)
{
    ...
    if (living(me) && me->query_temp("fanzhen") && random(skill) + 30 > ob->query_skill("parry", 1) / 2 && me->query("combat_exp") > ob->query("combat_exp") / 2 && skill > 199 && neili > 500 && living(ob) && !ob->query("env/invisibility"))
    {
        if (!objectp(weapon = ob->query_temp("weapon")))
        {
            ...
            else
            {
                ...
                if (p < 1)
                    p = 1;
                p *= damage / 2;
                if (ob->query_skill("force") > me->query_skill("force") * 3 / 2)
                    p = p * 2 / 3;
                if (p < 10)
                    p = 10;
                ...
                if (wizardp(me))
                    tell_object(me, "Fanzhen damage: " + p + "。\n");
