...
{
        return notify_fail("独孤九剑只能用学(learn)的来增加熟练度。\n");
}
*/
int valid_learn(object me)
{
        ...
        if ((int)me->query_skill("jiuyang-shengong", 1) > 1)
                return notify_fail("你觉得自身所学的九阳神功与独孤九剑冲突不已，根本没办法共存。\n");

        if ((int)me->query_skill("never-defeated", 1) > 1)
                return notify_fail("你觉得自身所学的不败神功与独孤九剑冲突不已，根本没办法共存。\n");   

        if ((int)me->query_skill("kuihua-mogong", 1) > 1)
                return notify_fail("你觉得自身所学的葵花魔功与独孤九剑冲突不已，根本没办法共存。\n");                
...
int practice_skill(object me)
{
        ...
        if ( level > 400 ) cost = level + random(level);
                     else  cost = level / 3 + random(level);	
...
int difficult_level()
{
       if ( this_player()->query_skill("lonely-sword", 1) > 600)
       return 1100;
       else   return 600;
}