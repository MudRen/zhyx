...
int perform(object me, object target)
{
	...
	msg = HIM "$N" HIM "招数忽然的一变，摹可明方，内力四处回旋，无形中牵引着$n"
        ...
        ap = level + me->query_skill("force") * 2 / 3 + me->query_skill("jiuyang-shengong",1);
        dp = target->query_skill("force") * 2 / 3 + target->query_skill("dodge", 1) + target->query_skill("jiuyang-shengong",1);
        if (level >= 300)
        {
                // layer 7 force
                ap += level;
