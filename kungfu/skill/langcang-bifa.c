...
int valid_learn(object me)
{
        ...
        if ((int)me->query_skill("dagger", 1) < (int)me->query_skill("langcang-bifa", 1))
                return notify_fail("你的基本短兵水平有限，无法领会更高深的浪沧笔法。\n");
...
mapping query_action(object me, object weapon)
{
        ...
        level = (int) me->query_skill("langcang-bifa", 1);
...
int practice_skill(object me)
{
        ...
        if (! objectp(weapon = me->query_temp("weapon")) ||
            (string)weapon->query("skill_type") != "dagger")
                return notify_fail("你所持的武器无法练习浪沧笔法。\n");

        if ((int)me->query("qi") < 70)
                return notify_fail("你的体力不够练浪沧笔法。\n");

        if ((int)me->query("neili") < 70)
                return notify_fail("你的内力不够练浪沧笔法。\n");
...
{
        return __DIR__"langcang-bifa/" + action;
}
