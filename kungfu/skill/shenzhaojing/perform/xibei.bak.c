...
int perform(object me, object target)
{
	...
	ap = me->query_skill("shenzhaojing",1) + me->query_skill("force") + me->query_skill("martial-cognize", 1);
	dp = target->query_skill("dodge",1) + target->query_skill("force") + target->query_skill("martial-cognize", 1);

	attack_time = 1 + (int)(me->query_skill("zuoyou-hubo", 1) / 80);
	...
	for (i = 0; i < attack_time; i++)
	{
		...
		message_combatd(HIW "$N" HIW "无声无影击出一拳，一股浑厚至极的内劲汹涌澎湃，" 
                     "带着排山倒海之势向$n" HIW "席卷而至。" NOR, me, target);
