...
/*        if (userp(target))
	...
	//石头
        ap = me->query_skill("yeqiu-quan", 1) + me->query_str() * 2 + me->query_skill("force", 1);
	...
	//剪刀
        ap = me->query_skill("yeqiu-quan", 1) + me->query_dex() * 2 + me->query_skill("dodge", 1);
	...
	//布
        ap = me->query_skill("yeqiu-quan", 1) + me->query_con() * 2 + me->query_skill("parry", 1);
        dp = target->query_skill("parry") + target->query_con() * 2;
	...
	msg = HIM "\n$N" HIM "使出三招以后，竟然越战越勇，对着$n"HIM"又是一阵穷追猛打！\n" NOR;
        ap = me->query_skill("yeqiu-quan", 1) + me->query_int() * 2;
        dp = target->query_skill("literate", 1) + target->query_int() * 2;
