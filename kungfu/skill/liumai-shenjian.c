...
{   
	...
	if ((int)me->query_skill("yiyang-zhi", 1) < 300 && (int)me->query_skill("canhe-zhi", 1) < 300)     
    return notify_fail("六脉神剑只能用学习或研究来增加熟练度。\n");   
        ...
        cost = me->query_skill("liumai-shenjian", 1) / 2;
...
mixed hit_ob(object me, object victim, int damage_bonus, int i, int attack_time)
{
	...
	//增加转世以后的damge和特效
	if (me->query("reborn"))
		damage_bonus += damage_bonus * ((int)me->query_skill("jingluo-xue", 1) / 250);
...
int difficult_level()
{
    ...
    if (me = this_player())
    {
       if ((int)me->query_skill("yiyang-zhi", 1) > 299 || (int)me->query_skill("canhe-zhi", 1) > 299)
       return 500;
	}
       return 800;
}
