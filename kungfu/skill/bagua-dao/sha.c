...
int perform(object me, object target)
{
        ...
        msg = HIR "$N" HIR "仰天狼啸，手中" + weapon->name() +
              HIR "斜划出一道半弧，整个刀身顿时腾起凛冽无比的"
              "杀气，直贯$n" HIR "而去！\n" NOR;

        //ap = me->query_skill("blade") + me->query_str() * 10;
        ap = me->query_skill("blade") + me->query_str() * 10 + random((int)me->query_skill("wai-bagua",1));
        	...
        	if (ap *2/3 + random(ap) > dp)
        {
                //damage = ap / 2 + random(ap);
                 damage = ap  + random(ap)+random((int)me->query_skill("nei-bagua",1));
                me->add("neili", -200);
                me->start_busy(3);
                msg += COMBAT_D->do_damage(me, target, WEAPON_ATTACK, damage, 80,
                                           HIR "只听$n" HIR "一声惨叫，$N" HIR +
                                           weapon->name() + HIR "刀光一闪，竟已嵌"
                                           "入$p" HIR "体内尺许，带出一蓬血雨。\n"
                                           NOR);
        } else
        {
