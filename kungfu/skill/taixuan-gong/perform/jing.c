...
int perform(object me, object target)
{
        ...
        if (flag)ap = me->query("int")*3 + lvl + me->query_skill("sword", 1) + me->query_skill("martial-cognize", 1) + me->query_skill("tianchi-neigong", 1)/2;
        else ap = me->query("int")*3 + lvl + me->query_skill("blade", 1) + me->query_skill("martial-cognize", 1) + me->query_skill("tianchi-neigong", 1)/2;

        // 第一招，判断对方臂力
        dp = target->query("str") * 3 + target->query_skill("unarmed", 1) + 
             target->query_skill("parry", 1) + target->query_skill("martial-cognize", 1);
        ...
        // 第二招，判断对方悟性
        dp = target->query("int") * 3 + target->query_skill("dodge", 1) 
             + target->query_skill("parry", 1) + target->query_skill("martial-cognize", 1);
        ...
        // 第三招，判断对方根骨
        dp = target->query("con") * 3 + target->query_skill("force", 1) + 
             target->query_skill("parry", 1) + target->query_skill("martial-cognize", 1);
        ...
        // 第四招，判断对方身法
        dp = target->query("dex") * 3 + target->query_skill("dodge", 1) + 
             target->query_skill("parry", 1) + target->query_skill("martial-cognize", 1);
