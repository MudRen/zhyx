...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
	...
	if (mp > 100)
		dp = me->query_skill("qiankun-danuoyi", 1) + me->query_skill("shenghuo-shengong", 1) / 4 + me->query_skill("count", 1) / 2; //弱化套装带来的count
	else
		dp = me->query_skill("qiankun-danuoyi", 1) + me->query_skill("shenghuo-shengong", 1) / 4;
...
int practice_skill(object me)
{
	...
	if ((int) me->query_skill("qiankun-danuoyi", 1) > 320 && (int) me->query_skill("shenghuo-shengong", 1) > 299)
		{
			if ((int)me->query("qi") < 300)
                return notify_fail("你的体力太低了。\n");

        cost = me->query_skill("qiankun-danuoyi", 1) / 2;

        if ((int)me->query("neili") < cost)
                return notify_fail("你的内力不够练大挪移神功。\n");
        ...
        return 1;
				...
				return notify_fail("以你目前对大挪移神功的领悟，只能通过学习(learn)来增加熟练度。\n");   
}

mapping *action = ({
([      "action":"$N运起乾坤大挪移，身前仿佛有一道气墙，仿佛让天下所有攻击都失去效果"
]),
});
