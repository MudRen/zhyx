...
int get_finish(object me)
{
        ...
        if (me->query("character") != "光明磊落")
                return notify_fail("你天性不符，无法领悟九阳神功。\n");
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
		...
		level=(int)me->query_skill("jiuyang-shengong", 1);
                if (level<100) backdamage=0;
                else if (level<220) backdamage=(int)damage/3 ;
                else if (level<350) backdamage=(int)damage/2 ;
                  else backdamage=damage;

        if (me->query_skill_mapped("parry") != "jiuyang-shengong"
        ...
        if (ap * 2 / 5 + random(ap) < dp)
        {
			if (random(3) != 1 )
			{
				...
				ob->receive_wound("qi", backdamage, me);
                        ...
                        result += (["msg" : HIY "$n" HIY "默念九阳神功总诀，以刚"
                                            "对刚，承下$N" + HIY "这一招，结果$N被$n以内力反震，「嘿」地一声退了两步！\n"NOR]);                        
                return result;
			}
		    else
			{
