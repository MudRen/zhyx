...
mixed ask_mieshi() 
     { 
             ...
             me = this_player(); 
             if (me->query("can_perform/canhe-zhi/mieshi")) 
                     return "这招老夫不是已经教过你了吗？下去多多练习！把我们慕容家发扬光大！"; 
      
             if (me->query("family/family_name") != query("family/family_name")) 
                     return "你是哪儿来的？找死么？"; 
      
             if (me->query("con") < 51)
                     return "你的先天根骨太低，就算领会了也难以施展。"; 

             if (me->query("str") < 47)
                     return "你的先天臂力太低，就算领会了也难以施展。"; 

             if (me->query("shen") > -1300000) 
                     return "你这人手段不够狠毒，我现在还不能传你这招！"; 

             if (me->query("gongxian") < 20000) 
                     return "你对门派的贡献还不够，我现在还不能传你这招！"; 
      
             if (me->query_skill("canhe-zhi", 1) < 800) 
                     return "你先好好把参合指练好了再说！"; 
      
             message_vision(HIY "$n" HIY "哼了一声！说罢挥" 
                            "手横扫，十指纷弹，顷刻间剑气漫天！$N" 
                            HIY "只觉得似要被眼前剑气穿体而亡，过了半会才从恐惧中回过神来。\n" NOR, 
                            me, this_object()); 
             command("heng"); 
             command("say 这便是我慕容绝学参合指终极绝技--灭世，记得把它发扬光大！"); 
             tell_object(me, HIG "你学会了灭世。\n" NOR); 
             if (me->can_improve_skill("finger")) 
                     me->improve_skill("finger", 1500000); 
             if (me->can_improve_skill("canhe-zhi")) 
                     me->improve_skill("canhe-zhi", 1500000); 
             if (me->can_improve_skill("martial-cognize")) 
                     me->improve_skill("martial-cognize", 1500000); 
             me->set("can_perform/canhe-zhi/mieshi", 1); 
