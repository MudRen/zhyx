...
mixed ask_sanjue()
{
        ...
        me = this_player();
        if (me->query("can_perform/damo-jian/sanjue"))
                return "自己好好多练习吧，牢记慈悲为怀，勿得乱用！";
        ...
        if (me->query_skill("damo-jian", 1) < 160)
                return "你的达魔剑法还不到家，无法领会其中奥妙！";
        ...
        message_vision(HIY "$n" HIY "看看$N" HIY
                       ...
                       me, this_object());
        command("say 记住，除非降妖除魔万不得已，勿要轻易使用。");
        tell_object(me, HIG "你学会了达摩三绝剑。\n" NOR);
        if (me->can_improve_skill("force"))
                me->improve_skill("force", 160000);
        me->set("can_perform/damo-jian/sanjue", 1);
        return 1;
}
...
mixed ask_lunhuijian()
{
	...
	if (me->query("reborn"))
		return "你已经转世为人，这套剑法不适合你使用！";
      	

	if (! me->query("can_learn/damo/lunhui-jian"))
	{
		me->set_skill("lunhui-jian", 1);
		me->set("can_learn/damo/lunhui-jian",1);
		return "你学会了『释迦轮回剑』！";
	}

	if (me->query_skill("lunhui-jian", 1) < 200)
		return "你的轮回剑法还不够两百级，无法去领悟绝招！";

	message_vision(HIY "$n" HIY "看看$N" HIY
                       "，道：“好吧，既然这样，你就去试试吧。说罢轻轻一推$N。\n" NOR,
                       me, this_object());

	    room = find_object("/d/special/taixu");
        if (! room) 
        	room = load_object("/d/special/taixu");
