...
inherit CLASS_D("generate") + "/chinese_maxultra";
...
void create()
{
	...
	set("can_perform/xueshan-jian/times", 2000000);

        set_temp("apply/armor", 400);
        set_temp("apply/damage",350);
        set_temp("born_time", time());
          set_temp("dest_time", 2000 + time());
...
void set_from_me(object me)
{
	...
	//限制知识类技能的等级
	if (query_skill("chess",1) >900)	set_skill("chess", 500);
	if (query_skill("count",1) >900)	set_skill("count", 500);
	if (query_skill("taoism",1) >900)	set_skill("taoism", 500);
	if (query_skill("buddhism",1) >900)	set_skill("buddhism", 700);
	if (query_skill("tanqin-jifa",1) >900)	set_skill("tanqin-jifa", 400);
	if (query_skill("jingluo-xue",1) >900)	set_skill("jingluo-xue", 400);
	if (query_skill("qimen-wuxing",1) >900)	set_skill("qimen-wuxing", 400);
...
void do_continue_help(object ob)
{
        ...
        while (n--)
        {
                obs[n] = new(CLASS_D("generate") + "/killed_maxultra.c");
