...
mixed ask_skill11()
{
        ...
        if (me->query("can_perform/longxiang-gong/wuji"))
                return "我不是已经教过你了么？还罗嗦什么。";
        ...
        if (me->query_skill("longxiang-gong", 1) < 420)
                return "你的龙象般若功修为不足，练练再来吧。";

        if (me->query_skill("force", 1) < 420)
                return "你的内功修为不足，难以领悟此招。";

        if (me->query_skill("unarmed", 1) < 420)
                return "你的基本拳脚修为不足，难以领悟此招。";

        message_sort(HIY "\n$n" HIY "面容肃穆，双手成拳" HIY
                     "，突然向前击出，内息澎湃"
                     "汹涌之极，犹如江河绝堤，便欲冲关破出一般，天空似"
                     "有十龙十象虚影向$N" HIY "碾压过来。\n\n" NOR, me, this_object());

        command("haha");
        command("say 刚才我这一招，你看明白了多少？");
        tell_object(me, HIC "你学会了「般若无极」。\n" NOR);
        ...
        if (me->can_improve_skill("longxiang-gong"))
                me->improve_skill("longxiang-gong", 1500000);
        me->improve_skill("martial-cognize", 1500000);
        me->set("can_perform/longxiang-gong/wuji", 1);
        me->add("gongxian", -4000);
