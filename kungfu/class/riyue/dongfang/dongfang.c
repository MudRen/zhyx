...
{       
        ...
        if (me->query_skill("kuihua-mogong", 1) < 300)
        {
               command("say 你葵花魔功还不够娴熟，还不快下去多加练习。");
               return 1;
        }
        ...
        tell_object(me, HIG "你学会了「无双无对」的「无双」式。\n");
...
void attempt_apprentice(object ob)
{
        ...
        if (ob->query_skill("xixing-dafa", 1) < 200||
		ob->query_skill("riyue-xinfa", 1) < 200||
		ob->query_skill("tianhuan-shenjue", 1) < 200||
		ob->query_skill("sword", 1) < 200)
        {
                command("hmm");
                command("say 你的武功这么差，怎么能成大事。");
                return;
        }
        command("nod2");
        command("say 你背弃任我行，忠心于我，今日我便收你为徒。");
        command("say 我就让你作我的护教长老，待我夺了教主之位后成就千秋大业！");
        command("recruit " + ob->query("id"));
	ob->set("title",HIY"日月神教护教长老"NOR);
        return;
}
