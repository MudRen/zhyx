...
mixed ask_skill5()
{
        ...
        if (me->query("can_perform/daya-jian/bugui"))
                return "咦？这招你不是已经会了吗？";

        if (me->query("family/family_name") != query("family/family_name"))
                return RANK_D->query_respect(me) + "和本派素无瓜葛，何出此言？";

        if (me->query_skill("daya-jian", 1) < 1)
                return "你连大雅剑都没学，还谈什么绝招可言？";
        ...
        if (me->query_skill("daya-jian", 1) < 250)
                return "你的大雅剑修为还不够，自己下去练练再来吧。";

        message_vision(HIY "$n" HIY "微微一笑，转身从弦琴上取出一本"
                       "剑经，指着其中一段对$N" HIY "细说\n良久，$N"
                       HIY "一边听一边点头。\n" NOR, me, this_object());
        command("nod");
        command("say 既然如此，我就传你这招。");
        tell_object(me, HIC "你学会了「不归式」。\n" NOR);

        if (me->can_improve_skill("sword"))
                me->improve_skill("sword", 1500000);
        if (me->can_improve_skill("daya-jian"))
                me->improve_skill("daya-jian", 1500000);
        me->improve_skill("martial-cognize", 1500000);
        me->set("can_perform/daya-jian/bugui", 1);
...
mixed ask_skill6()
{
        ...
        if (me->query("can_perform/daya-jian/feng"))
                return "咦？这招你不是已经会了吗？";
        ...
        if (me->query_skill("daya-jian", 1) < 180)
                return "你的大雅剑修为还不够，自己下去练练再来吧。";
        ...
        if (me->can_improve_skill("daya-jian"))
                me->improve_skill("daya-jian", 1500000);
        me->improve_skill("martial-cognize", 1500000);
        me->set("can_perform/daya-jian/feng", 1);
