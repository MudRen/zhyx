...
mixed ask_skill9()
{
        ...
        if (me->query("can_perform/zhemei-shou/zhua"))
                return "你自己下去练习吧。";
        ...
        if (me->query("int") < 34)
                return notify_fail("你先天悟性不足，无法学习灵巧的终极绝招。\n");
        ...
        if (me->query_skill("zhemei-shou", 1) < 250)
                return "你的逍遥折梅手火候未到，尚且学不了这一招。";
        ...
        if (me->can_improve_skill("beiming-shengong"))
                me->improve_skill("beiming-shengong", 1500000);
        me->improve_skill("martial-cognize", 1500000);
        me->set("can_perform/zhemei-shou/zhua", 1);
