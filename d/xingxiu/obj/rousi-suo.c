...
void init()
{
	...
	mapping myfam;
        me = this_player();
        if (!objectp(me) || !userp(me) || !me->query("family")) return; 
	myfam = me->query("family");
        if (!mapp(myfam)) return;
...
int do_shou(string arg)
{
		...
		return notify_fail("你不激发飞星术，无法掌握柔丝索的使用方法。\n");

        if(me->query_skill("feixing-shu",1)<30)
		return notify_fail("以你现在的飞星术功力恐怕用不了柔丝索！\n");

        if(ob->query_temp("catched"))
                {
                victim=find_player(ob->query_temp("catched"));
                if (victim && environment(victim)->short()=="柔丝索中")
...
int do_look(string arg)
{
        ...
        if( !arg || (arg != "suo" && arg !="rousi suo" && arg != "rope")) return 0;
