...
int do_put(string arg)
{
        ...
        if( !objectp(obj = present(item, me)) )
	{
        ...
        if (obj->query("name") != "点燃的香" || obj->is_character() || obj->is_item_make()) 
	{
		write("你不能把这样东西放进神木王鼎。\n");
		return 1;
	}
...
int do_suck(string arg)
{
	...
	if ( me->query_skill("hunyuan-yiqi",1) || me->query_skill("linji-zhuang", 1)
        ...
        || me->query_skill("taiji-shengong",1) || me->query_skill("zixia-shengong",1)
	|| me->query_skill("jinyan-gong", 1) || me->query_skill("kurong-changong", 1))
	...
	&& me->query("body_poison") > (int)me->query_skill("poison",1)/10)
        ...
        switch( random(3) )
        {
			...
			SKILL_D("chousui-zhang")->valid_learn(me))
                {
                ...
                break;
            case 1:
                if ( (int)me->query_skill("sanyin-wugongzhao", 1) > 30 &&
			SKILL_D("sanyin-wugongzhao")->valid_learn(me))
                {
                        me->improve_skill("sanyin-wugongzhao", me->query_int());
                        write("你的手指上隐隐罩上一层深紫色，「三阴蜈蚣爪」修为加深了。\n");
                        me->set_temp("muding_learned",1);
                }
                break;
	    case 2:
		if (me->query_temp("muding1") || (query("canblood") > 0 && SKILL_D("poison")->valid_learn(me)))
                {
...
int open_up(object room)
{
	...
	ob->move(obj);
	message("vision", RED "忽听得地上嗦嗦声响，却是一只"+ob->name()+"爬了过来。那"+ob->name()+"闻到鼎中散出的香气，径身游向木鼎，从鼎下的孔中钻进去，便不再出来。\n" NOR, environment(obj));
