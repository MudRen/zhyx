...
int accept_object(object me, object ob)
{
		...
		{
                command("say 你这水平怎么会有这东西，莫非是偷来的？");
                command("heng");
                command("say 东西没收，下不为例！");
...
mixed ask_me2()
{
        ...
        me = this_player();
        if (me->query("can_perform/bizhen-qingzhang/yong"))
                return "能教的已经教完了，还来找我干嘛？";

        if (! me->query("can_learn/xieyanke/bizhen-qingzhang"))
                return "给我滚开，少在我面前罗嗦！";

        if (me->query_skill("bizhen-qingzhang", 1) < 220)
                return "你的碧针清掌练这个程度就来问绝招？";

        message_vision(HIW "$n" HIW "冷笑一声，双掌连续挥舞"
                       "出一个又个大圈，顿时一股又一股汹涌澎"
                       "湃的劲气如波浪一般向" HIW "涌来。\n$N"
                       HIW "大骇，硬接了这一招，只感内息翻腾"
                       "，全身骨骼快要散架一般。突然目光一亮"
        ...
        tell_object(me, HIC "你学会了碧针清掌「汹涌」这一招。\n" NOR);
        if (me->can_improve_skill("strike"))
                me->improve_skill("strike", 300000);
        me->set("can_perform/bizhen-qingzhang/yong", 1);
        return 1;
}
