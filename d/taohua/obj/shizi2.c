...
//	if( exp/3+random(exp/2) > exp2 /2 && skill > skill2){
	if(skill > skill2){
		...
		if( !victim->query_temp("weapon") ) {
			if(jiali >= 240) 
				message_vision(HIY"$n刚听到一点声息，小石子转瞬间划过长空，已击中$n"+limb+"，先声夺人，威不可当！\n" NOR, me, victim);
			else if(jiali >= 200) 
				message_vision(HIY"但听小石子破空之声响亮异常，来势比铁胎弹弓所发还要急劲，转瞬间已击中$n"+limb+"！\n" NOR, me, victim);
			else 
				message_vision(HIY"$n被打个措手不及，虎口登时鲜血长流，手臂再也抬不起来。\n" NOR, me, victim);
        	...
        	{ 
			message_vision(HIY"小石子破空飞至，撞在$N手中的" + weapon->name() + HIY"上。\n"NOR,victim);
