...
===============================================================
      为感谢新老玩家的支持，本站决定在2003年10月1日前，新注册玩
      家可以使用 newwash 臂力 悟性 根骨 身法 来指定你的属性！
===============================================================
...
void init()
{
        ...
        add_action("do_newwash", "newwash");
}

int do_newwash(string arg)
{
        mapping my;
        int str, inte, con, dex, all;
        object me;
        ...
        if (! arg)return notify_fail("指令格式：newwash 臂力 悟性 根骨 身法 \n");

        if (sscanf(arg, "%d %d %d %d", str, inte, con, dex) != 4)
                 return notify_fail("指令格式：newwash 臂力 悟性 根骨 身法 \n");

        if (str < 13 || str > 30 || inte < 13 || inte > 30 ||
            con < 13 || con > 30 || dex < 13 || dex > 30)
                return notify_fail("输入值必须不低于13，不超过30。\n");

        all = str + inte + con + dex;
        if (all != 80)return notify_fail("四个先天属性值之和必须等于80。\n");
...
int do_wash(string arg)
{
        ...
        if (! arg)return notify_fail("指令格式：wash 臂力 悟性 根骨 身法 \n");

        if (sscanf(arg, "%d %d %d %d", str, inte, con, dex) != 4)
                 return notify_fail("指令格式：wash 臂力 悟性 根骨 身法 \n");
