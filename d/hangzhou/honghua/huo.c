...
mixed ask_skill1()
{
        ...
        if (me->query_skill("sword", 1) < 350)
                return "你的剑术修为不够，我还不能教你「海市蜃楼」！";

        if (me->query_skill("dodge") < 350)
                return "你的轻功修为不够，我还不能教你「海市蜃楼」！";

        if ((int)me->query("dex") < 45)
                return "你的身法灵动性太差，难以领悟「海市蜃楼」！";
...
{
        if (ob->query("id") != "jimao letter")
        {
                command("say 你给我这种东西干嘛？");
                return 0;
        } else
        {
		if(me->query("reborn"))
		{
                command("say 好吧！");
                command("say 我这里有一套三分剑术，这就传授与你。");

                if (me->query_skill("sanfen-jianshu", 1) < 50)
                me->set_skill("sanfen-jianshu", 10); 

                tell_object(me, HIW "你学会了三分剑术！\n" NOR); 
                command("say 下去好好练习吧。"); 
                destruct(ob);
                return -1;
		} else
		{
                command("say 咦，你这东西哪来的？");
                command("heng");
                command("say 来历不明，正好也物归原主，东西我就没收了！");
