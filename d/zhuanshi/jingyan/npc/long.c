...
void create()
{
        set_name(HIR"恶龙"NOR, ({ "e long" }) );
        set("race", "野兽");
        set("gender", "雄性");
        set("age", 200);
        set("long", 
"这是一条全身赤红，吸天地之间的灵气而长大的恶龙！
它全身散发着赤红色的火焰。\n");
...
int big_blowing()
{
        ...
        message_vision( HIR "\n\n恶龙深深地吸入一口气，全身转为鲜红的火色，整个恶龙胀大了几倍！！\n\n" NOR,
                        this_object());
...
void die()
{
         ...
         if(objectp(killer=this_object()->query_last_damage_from()))
        {
                ...
                if (!killer->query_temp("m_success/恶龙"))
                {
                        killer->set_temp("m_success/恶龙",1);
                        killer->add("combat_exp",exp);
                        killer->add("potential",20000);
                        message("channel:chat", HBMAG"【转世】"+killer->query("name")+"成功地杀死了恶龙!得到宝物金刚圈!\n"NOR,users());
                }
                
                tell_object(killer, HIR "\n\n一团火焰猛地升腾，一个声音说道：谢谢你杀死恶龙，你将得到金刚圈作为奖励！！！\n" NOR); 
