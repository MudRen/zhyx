...
int do_canwu(string msg)
{
    ...
    if (random(30) == 1 && witch == "白首太玄经")
    {
        me->add("taixuan_perform/jing/count", 1);
        if (me->query("taixuan_perform/jing/count") >= 100)
        {
            write(HIM "猛然间，你一声长啸，胸中豁然贯通，再无疑虑。\n" NOR);
            write(HIC "你终于通晓了太玄功至高绝学「白首太玄经」。\n" NOR);
            me->set("can_perform/taixuan-gong/jing", 1);
            me->delete("taixuan_perform/jing/count");
            if (me->can_improve_skill("force"))
                    me->improve_skill("force", 2500000);
            if (me->can_improve_skill("taixuan-gong"))
                    me->improve_skill("taixuan-gong", 2500000);
            if (me->can_improve_skill("martial-cognize"))
                    me->improve_skill("martial-cognize", 2500000);
            return 1;
        }
