...
#define DEFAULT_EFUN_PATHS ({"/doc/help/", "/doc/skill/", "/doc/applies/", "/doc/efuns/",})

#define DEFAULT_HELP_PATHS ({ "/doc/help/",})
...
int main(object me, string arg)
{
    ...
    if( !arg ) {
             //   if (file_size(HELP_DIR + "/topics") > 0)
                if (file_size("/doc/help/topics") > 0)
                {
               // me->start_more(color_filter(read_file(HELP_DIR + "/topics")));
                        me->start_more(color_filter(read_file("/doc/help/topics")));
                } else
                {
                        write("没能找到帮助主题。\n");
                }
