...
#define PREFIX(ob)    (ob->is_chatter() ? HIC "~" : interactive(ob) ? (query_idle(ob) > 120 ? HIG "*" NOR : " ") : stringp(ob->query("doing")) ? HIG " " NOR : HIR "#" NOR)
...
mixed main(object me, string arg, int remote)
{
	...
	string who_id;
        ...
        if (arg)
        {
                ...
                while (i--)
                        switch (option[i])
                        {
                                ...
                                case "-i":
                                case "-g": opt_newhelper=1;		break;
                                case "-s": opt_sort = 1;        break;
                                case "-S": opt_sort = -1;       break;
                                case "-n": opt_number = 1;      break;
                                case "-close": opt_closed = 1;      break;
        ...
        if (opt_newhelper)
                ob = filter_array(ob, (: $1->query("newhelper") == 1 :));
	...
	if (objectp(vob))
        ...
        if (opt_wiz)
                str += (opt_female ? "女性" : opt_male ? "男性" : "") +
                       "巫师" + who_name + "有：";
        if (opt_newhelper)
                        str += (opt_female ? "女性" : opt_male ? "男性" : "") +"新手导师有:";
...
{
        write("
 指令格式 : who [-h] [-l] [-w] [-p] [-c] [-fam] [-f] [-m] [-s] [-S] [<ID>] [-n] [-g] [is <中文名字>]
