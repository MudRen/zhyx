...
int main(object me, string arg)
{
	...
	int lvl;
        ...
        if (where->query("trds"))
                return notify_fail("你还是等屠人大赛结束后才作研究吧。\n");
	...
	if (! SKILL_D(skill)->valid_learn(me) || lvl < 180) return 0;

        if (!me->query("tianmo_jieti") && !XUE_CMD->can_learn(me, skill))
                return 0;
