...
int main(object me, string arg)
{
      ...
      /*  if (where->query("no_fight") && me->query("doing") != "scheme")
        ...
        c = 150 + me->query_temp("apply/research_times");
        // 特殊物品增加研究次数
        ...
        if (lvl >= 1000)
                improve += lvl / 300 + me->query("int") / 10;
        else
        if (lvl >= 800)
                improve += lvl / 200 + me->query("int") / 10;
        else
        if (lvl >= 600)
                improve += lvl / 150 + me->query("int") / 10;
        else
        if (lvl >= 500)
	...
	{
	if (me->query("reborn"))
		improve += improve * 5 / 100; //转世id增加5%研究速度
	}
