...
object offensive_target(object me)
{
    ...
    if (!me || !me->query_enemy() || !arrayp(me->query_enemy()))
        return 0;
...
void leave()
{
    ...
    {
        if (environment())
            message("vision", HIB + name() + "身子一晃笑道:还没人来吗?那我走了。\n" NOR, environment());
...
void die()
{
    ...
    if (owner = killer->query("id"))
    {
        ...
        if (myexp < 500000)
            ...
            i = (to_int(sqrt(to_float(myexp / 100000 - 1)))) - 1;
        if (i > 10)
            i = 10;
        ...
        tell_object(owner_ob, "你得到了" + chinese_number(exp_r * 8) + "点武学经验和" + chinese_number(pot_r * 5) +
                    "点潜能!体会和阅历,贡献也有所提高\n");
...
int attack()
{
    ...
    if (objectp(opponent))
    {
        ...
        if (opponent->query("id") == owner && userp(opponent) && !living(opponent))
        {
            //huarong 2004/7
...
*/
    if (i >= 5 && random(4) == 1)
        command("exert recover"); //玩家500万exp以后，怪开始随机吸气

    if (query("family/family_name") == "欧阳世家")
    {
        if (!ob->query_temp("powerup") && me->query_skill("force", 1) > 249)
        ...
        case 1:
            ...
            command("perform puji");
            break;
        case 2:
            command("perform hama " + me->query("id"));
            command("perform hama");
            break;
        case 3:
            command("perform puji " + me->query("id"));
            command("perform puji");
    ...
    if (query("family/family_name") == "少林派")
    {
        ...
        case 0:
            ...
            command("perform sword.sanjue");
            break;
        case 1:
            command("perform sword.shang " + me->query("id"));
            command("perform sword.shang");
            break;
        case 2:
            command("perform sword.luan " + me->query("id"));
            command("perform sword.luan");
            break;
        case 3:
            command("perform sword.sanjue " + me->query("id"));
            command("perform sword.sanjue");
        ...
        case 4:
            ...
            command("perform strike.san");
            break;
        case 5:
            command("perform sword.zhe " + me->query("id"));
            command("perform sword.zhe");
            break;
        case 6:
            command("perform sword.sanjue");
            break;
    ...
    {
        if (!ob->query_temp("powerup") && me->query_skill("force") > 249)
            command("yun powerup");
        if (k > 7)
            k = 7;
        switch (k)
        ...
        case 3:
            ...
            command("perform throwing.wuying");
            break;
        case 4:
            command("perform sword.lian " + me->query("id"));
            command("perform sword.lian");
    ...
    {
        if (!ob->query_temp("powerup") && me->query_skill("force") > 149)
            command("yun powerup");
        if (k > 7)
            k = 7;
        switch (k)
        ...
        case 2:
            ...
            command("perform sword.jia");
            break;
        case 3:
            command("perform sword.zhuan " + me->query("id"));
            command("perform sword.zhuan");
            break;
        default:
            return 0;
        }
    ...
    if (query("family/family_name") == "峨嵋派")
    {
        ...
        case 0:
            ...
            command("perform sword.mie");
            break;
        case 1:
            command("perform sword.jue " + me->query("id"));
            command("perform sword.jue");
            break;
        case 2:
            command("perform sword.mie " + me->query("id"));
            command("perform sword.mie");
    ...
    if (query("family/family_name") == "逍遥派")
    {
        ...
        case 1:
            ...
            command("perform sword.liu");
            break;
        case 2:
            command("perform hand.hai " + me->query("id"));
            command("perform hand.hai");
            break;
        case 3:
            command("perform strike.huan " + me->query("id"));
            command("perform strike.huan");
            break;
        case 4:
            command("perform sword.liu " + me->query("id"));
            command("perform sword.liu");
    ...
    if (query("family/family_name") == "丐帮")
    {
        ...
        case 0:
            ...
            command("perform staff.chuo");
            break;
        case 1:
            command("perform staff.chan " + me->query("id"));
            command("perform staff.chan");
            break;
        case 2:
            command("perform staff.ban " + me->query("id"));
            command("perform staff.ban");
            break;
        case 3:
            command("perform staff.ban " + me->query("id"));
            command("perform staff.ban");
    ...
    if (query("family/family_name") == "段氏皇族")
    {
        ...
        case 0:
            ...
            command("perform finger.dian");
            break;
        case 1:
            command("perform finger.jian " + me->query("id"));
            command("perform finger.jian");
            break;
        case 2:
            command("perform finger.qian " + me->query("id"));
            command("perform finger.qian");
            break;
        case 3:
            command("perform finger.dian " + me->query("id"));
            command("perform finger.dian");
    ...
    if (query("family/family_name") == "关外胡家")
    {
        ...
        case 0:
            ...
            command("perform blade.zhui");
            break;
        case 1:
            command("perform blade.xian " + me->query("id"));
            command("perform blade.xian");
            break;
        case 2:
            command("perform blade.cang " + me->query("id"));
            command("perform blade.cang");
            break;
        case 3:
        ...
        case 4:
            ...
            command("perform throwing.shan");
            break;
        case 5:
            command("perform throwing.xin " + me->query("id"));
            command("perform throwing.xin");
            break;
        case 6:
            command("perform dodge.fei");
            break;
    ...
    if (query("family/family_name") == "神龙教")
    {
        ...
        case 0:
            ...
            command("perform strike.hua");
            break;
        case 1:
            command("perform hand.xian " + me->query("id"));
            command("perform hand.xian");
            break;
        case 2:
            command("perform strike.hua " + me->query("id"));
            command("perform strike.hua");
    ...
    if (query("family/family_name") == "玄冥谷")
    {
        ...
        case 0:
            ...
            command("perform strike.ying");
            break;
        case 1:
            command("perform strike.zhe " + me->query("id"));
            command("perform strike.zhe");
            break;
        case 2:
            command("perform strike.lang " + me->query("id"));
            command("perform strike.lang");
            break;
        case 3:
            command("perform strike.ying " + me->query("id"));
            command("perform strike.ying");
    ...
    if (query("family/family_name") == "雪山派")
    {
        ...
        case 0:
            ...
            command("perform sword.hui");
            break;
        case 1:
            command("perform sword.chu " + me->query("id"));
            command("perform sword.chu");
            break;
        case 2:
        ...
        case 3:
            ...
            command("perform cuff.san");
            break;
        case 4:
            command("perform cuff.jue " + me->query("id"));
            command("perform cuff.jue");
            break;
        case 5:
            command("perform strike.piao " + me->query("id"));
            command("perform strike.piao");
            break;
        case 6:
            command("perform sword.hui");
            break;
