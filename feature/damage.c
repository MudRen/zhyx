...
varargs int receive_damage(string type, int damage, object who)
{
       ...
       //如果是skill呼叫该涵数则消除busy
        if (file_name(previous_object())[0..13] == "/kungfu/skill/")
...
void unconcious()
{
        ...
        me = this_object();
        if (! living(me)) return;
        if (( wizardp(me) || ! userp(me) ) && query("env/immortal")) return;
        if ( query_temp("no_die")) return;
