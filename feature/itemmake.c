...
#define	LEVEL8		10000
#define	LEVEL9		50000
#define	LEVEL10		90000
#define	LEVEL11		140000
#define	LEVEL12		200000
#define	LEVEL13		270000
#define	LEVEL14		350000
#define	LEVEL15		440000
...
string weapon_long()
{
        ...
        {
        case 1:
                if (attack_lvl >= LEVEL14)
                        result = HIY "它看上去平平常常，没有半点特殊，只是"
                                 "隐隐的让人感到那不凡的气质。\n" NOR;
                else
                if (attack_lvl >= LEVEL9)
        ...
        if (lvl)
        {
		if (query("magic/tessera") && lvl > 14)
		{
		        result += HIY + name() + HIY "的等级：无上神品 LV15\n" NOR;
		}
		else
		{
	                ...
	                if ((n = query("magic/imbue")) > 0 && !query("magic/tessera") )
...
{       string result;
        ...
        //天子等特殊护甲不返回等级描述
        if ( query("special") ) return "";

        result = HIC"可以被镶嵌物品！\n"NOR;
             result += HIW"防具圣化次数：" + query("bless") + " 。\n"NOR;
        d = query("dazao");
        if(d)
        {
