// newsd.c
//添加了新闻搜索功能
//Modified for txwd by mike 2005.5.4
#include <ansi.h>
#include <getconfig.h>
...
void show_news(object me, int raw)
{
        ...
        if (i == sizeof(notes) - 1)
        {
                tell_object(me, WHT "中华英雄总共发布过" HIY +
...
void do_read(object me, string arg)
{
        ...
        me->start_more(sprintf(HIC "≡" HIY "──────────────────"
                               ...
                               ctime(notes[num]["time"])) + notes[num]["msg"]);
...
return;
    	...
    	{
    			if(!( strsrch(notes[i]["msg"],content) == -1 ))
    				{
//    					reply+= "测试" + k + "\n";
    					reply += sprintf("%s [%3d]" NOR " %-" + (45 + color_len(notes[i]["title"])) +
                               "s %s" WHT "『 " CYN "%s" NOR + WHT " 』" NOR "\n",
