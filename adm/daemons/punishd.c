...
mapping family_punishers = ([
        ...
        "衡山派" : ({ CLASS_D("misc") + "/henshan" }),
        "绝情谷" : ({ CLASS_D("misc") + "/jueqing" }),
        "昆仑派" : ({ CLASS_D("misc") + "/kunlun" }),
        "凌霄城" : ({ CLASS_D("misc") + "/lingxiao" }),
        "梅庄" : ({ CLASS_D("misc") + "/meizhuang" }),
        "中原苗家" : ({ CLASS_D("misc") + "/miao" }),
        "明教" : ({ CLASS_D("misc") + "/mingjiao" }),
        "日月神教" : ({ CLASS_D("misc") + "/riyue" }),
        "山东商家" : ({ CLASS_D("misc") + "/shang" }),
	...
	//to fix the bug of phonix suit
        obs = filter_array(users(),
			(: $1->query("combat_exp") >= 10000 &&
			   ...
			   ! $1->query("special_skill/selfstudy") &&
			   ! $1->query_temp("phoenixeff") :));
        if (sizeof(obs))
	{
	        foreach (ob in obs)
        	{
		        CHANNEL_D->do_channel( this_object(), "sys", "清除异常的套装COUNT。");
			ob->delete_skill("count");
		}
	}
...
void family_punish()
{
        ...
        obs = filter_array(users(), (: $1->query("combat_exp") >= 100000 &&
                                       ...
                                       ! environment($1)->query("biwutai") &&  //擂台
