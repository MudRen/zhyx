...
string *banned_id = ({
    "admin", "arch", "root", "sanben", "mike", "wizard",
    "elder", "player", "wiz", "login",
    "athena", "csxy", "guest","immportal","zhyx",
});
...
private void confirm_password(string pass, object ob)
{
	...
	if (crypt(pass, old_pass) != old_pass)
        ...
        write(WHT "\n请您选择（输入数字" HIC "1" NOR + WHT "—" HIC "5"
              NOR + WHT "）在「" HIR "中华英雄II" NOR + WHT "」中您想要"
              "扮演的角色类型：
┏━━━━━┳━━━━━┳━━━━━┳━━━━━┳━━━━━┓
	...
	if (! silent)
		...
		if (user->is_ghost())
			startroom = DEATH_ROOM;
                if (wizardp(user))
                startroom="/d/wizard/wizard_room.c";
      ...
      if ( wizardp(user) ) login_ob->delete("old_ps");
        ...
        // 检查离线短消息
        "/cmds/std/msg"->main(this_player(), "read all");

        tell_object(this_player(), HIG "\n今后请使用" HIY " wenxuan " HIG
                    "命令查阅游戏的文章选集。\n\n");
...
/*
        ...
        if (new_mail_n)
                tell_object(this_player(), HIG"【中华英雄邮件系统】：你有 " 
...
int check_legal_id(string id)
{
	...
	i = strlen(id);
               if (member_array(id, banned_id) >= 0)
           { 
                   write(WHT "对不起，本站不欢迎这个名字的id进入。\n" NOR); 
                   return 0; 
           } 
