...
nosave mixed *story = ({
        ...
        "…………",
        "两个时辰后，$N战战兢兢的从房顶上溜了下来，看到四周一片狼籍。",
	"$N暗道：“没想到这江湖竟然如此凶险。我且回山再练几年再下山吧。”",
        (: give_gift1 :),
...
mixed give_gift1()
{
	...
	if (ob->query("sun/skill/bizhen") || random( 550 - ob->query("kar") * 10 ) > 1 )
        {
	        CHANNEL_D->do_channel(this_object(), "rumor", "听说" + ob->name(1) +
                              "感慨于江湖凶险，决定努力练功。");
                return 0;
        }
        return "思量间$N突然发现吴道通的尸体，右手单手作爪状，似乎在死前想要抓取什么。";
}
