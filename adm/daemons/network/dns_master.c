...
#ifdef DEBUG
    CHANNEL_D->do_channel(this_object(), "debug", sprintf("receive %s host:%s port:%s function: %s", (flag > 0) ? "valid" : RED "invalid" WHT, addr, args["PORTUDP"], func));
#endif
}
...
string start_message()
{
    return sprintf( "||MUDNAME:%s||NAME:%s||VERSION:%s||MUDLIB:%s||HOST:%s||PORT:%d"
            "||PORTUDP:%d||TIME:%s||USERS:%d||TCP:%s||UPTIME:%d", CHINESE_MUD_NAME,Mud_name(),
            MUDLIB_VERSION, MUDLIB_NAME, query_host_name(),
            mud_port(), my_port, ctime(time()), sizeof(users()), TCP_SERVICE_LEVEL,uptime());
}
...
{
    // check permissions
    if (!previous_object() || !ACCESS_CHECK(previous_object()) || file_name(previous_object())[0..strlen(AUX_PATH)-1] != AUX_PATH)
        return;

    // delete the entry
    map_delete(muds, name);
    //muds[name]["UPTIME"]="-100";//用于MUDLIST命令。
...
void support_q_callback(mapping info)
{
    ...
    if (!info || !info["CMD"] || !info["NAME"] || !strlen(info["CMD"]) || !strlen(info["NAME"]))
        return;
...
mapping query_mud_info(string name)
{
    ...
    name = htonn(name);
    if (name == mud_nname())
        return this_host + (["TIME":ctime(time())]) + (["UPTIME":sprintf("%d", uptime())]);
    return muds[name];
}
...
void resolve_callback(string address, string my_ip, int key)
{
    ...
    */
    if (!my_ip)
        this_host["HOSTADDRESS"] = "LOCALHOST";
    else
...
void create()
{
    ...
    // set up our own info
    this_host = (["MUDNAME":CHINESE_MUD_NAME,
                     "NAME":Mud_name(),
                    "ALIAS":Mud_name(),
                   "MUDLIB":MUDLIB_NAME,
