...
string make_color_string(string arg);
...
mapping channels = ([
        ...
        "chat": ([      "msg_speak": HIC "【纵横天下】%s：%s\n" NOR,
                        "msg_emote": HIC "【纵横天下】%s" NOR,
                        "msg_color": HIC,
                        "name"     : "闲聊",
                ]),
...
//                        return notify_fail("你必须在完成注册三十分钟以后才能使用频道， 这段时间看看 help newbie 吧")
        ...
        {
                msg = sprintf(channels[verb]["msg_speak"], who, wizardp(me) ? make_color_string(arg) : arg);
                message("channel:" + ((verb == "ultra") ? "chat" : verb), msg, obs);
                channel_log(msg, verb, me);
        }
