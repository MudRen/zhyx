...
#define AJSZ "/d/zhuanshi/ajszhuang/shanlu"

void jingyan_close(object n_place);
void qilin_close(object place);
void ajsz_close(object aj_place);
...
void create()
{
       ...
       //麒麟窟入口
        ...
        //ajsz入口
        remove_call_out("ajsz_open");
        call_out("ajsz_open", 850);
}
...
{       
     ...
     remove_call_out("jingyan_open");
     call_out("jingyan_open", 3600 * 2); //定三小时开一次口
...
void qilin_close(object room)
{
    ...
    remove_call_out("qilin_open");
    call_out("qilin_open", 3600 * 3); //定六小时开一次口
...
void ajsz_open()
{
     object room = find_object("/d/wudang/sheshenya");
     if (! room)
       room = load_object("/d/wudang/sheshenya");

    room->set("exits/ajsz", AJSZ);
    
     message("vision", HIG"【" HIR"江湖传言" HIG"】" HIW
                "：听说武当山山崖，突然出现一座.....。\n"NOR,users());
      
    remove_call_out("ajsz_close");
    call_out("ajsz_close", 600, room); //十分种后关闭
}

void ajsz_close(object room)
{
     if ( room && room->query("exits/ajsz"));
         room->delete("exits/ajsz");

     message("vision", HIG"【" HIR"江湖传言" HIG"】" HIW
                "：听说武当山雾气太大，神秘建筑再次消失了。\n"NOR,users());

    remove_call_out("ajsz_open");
    call_out("ajsz_open", 3600 * 2); //定六小时开一次口
   //call_out("ajsz_open", 1800); //测试时定半小时开一次口
