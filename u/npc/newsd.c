...
void show_news(object me, int raw)
{
        ...
        msg =HIY "目前纵横天下中的最新消息如下：                          " BRED HIW " 纵横天下巫师组 \n" NOR
             BBLU HIW "-------------------------------------------------------------------------------\n" NOR;
...
void auto_notice(object me)
{
        ...
        {
        case 0:
                msg = RED "一阵烟雾过后，新闻精灵出现在你的面前，手里"
                      "捏着一张破破烂烂的纸头，对你宣读起来。\n" NOR;
                break;
        case 1:
                msg = BLU "地上突然开了一个裂缝，冒出一个鬼鬼祟祟的新"
                      "闻精灵，对着你就喋喋不休的罗嗦起来。\n" NOR;
                break;
        case 2:
                msg = CYN "你忽然听见有人喊你，你回头一看，原来是新闻"
                      "精灵满头大汗的跑了过来，\n" NOR;
                break;
        case 3:
                msg = YEL "你刚打了一个哈欠，忽然前面冒出一股烟雾，散"
                      "去以后只见一个新闻精灵懒洋洋的看着你，口中念念有词。\n" NOR;
                break;
        default:
                msg = WHT "“啪啦”一声，新闻精灵狼狈不堪的摔在地上，"
                      "你连忙将他扶起。\n" NOR;
                break;
        }

        msg += HIM "新闻精灵告诉你来自神界的最新消息：\n" NOR;
        tell_object(me, msg);
        me->start_more(sprintf("[%d] %-" + (40 + color_len(notes[num]["title"])) +
                               "s %s (%s)\n----------------------------------------------------------------------\n",
                               num + 1, notes[num]["title"], notes[num]["author"],
