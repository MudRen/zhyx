...
void create()
{
	set_name("白晶晶",({"bai jingjing","jingjing","bai"}));
	set_weight(1);
	set("unit","张");
	set("gender", "女性");
	set("title", HIW"牌蛊精"NOR);
	set("inquiry",
	([
		"mj": (: to_say_play :),
		"麻将": (: to_say_play :),
...
void init()
{
   ...
   add_action("do_dump","drop");
   add_action("do_dump","da");
   add_action("do_dump","chu");
...
int do_addin()
{
        ...
        ||  me->query("id")==P4Data["Id"]){
            if (!Playing) message_vision("欠脚啊～～$N不停的哭哭啼啼.....\n",me);
       ...
       if (Playing){
                ...
                else  message_vision("我也要玩啦～～$N不停的哭哭啼啼.....\n",me);
                return 1;
        }
...
int do_dump(string str,object me)
{
        ...
        if (!str) return notify_fail("指令：dump <代码>或da <代码>或chu <代码>\n");
...
int do_eat(string str)
{
        ...
        if (Touched){
           command("beep "+me->query("id"));
           command("say 打牌啦！");
           return 1;
        }
        if (do_check_eat(num1,num2,LastDump)==0){
           command("benger "+me->name());
           command("say 你眼精花了ㄚ！这样也能吃？");
           return 1;
        }
        if (num1==num2){
           command("noway "+me->name());
           command("say 相同的牌不能吃。");
           return 1;
        }
        if (me->query("id")==P1Data["Id"]) who=P1Data;
...
int do_setmj(string str)
{
        ...
        if (str=="tin"){
           ...
           if (numstr=="none"){
              return notify_fail("不能取消自动听牌，要是设定错误，等着诈胡吧！:>。\n");
           }
...
例子3: setmj tin none         取消会自动胡牌设定。
       setmj gon none         取消所有自动杠牌设定。
       setmj pon none         取消所有自动碰牌设定。
...
　　每胡就有底台５，在加上所得的台就是总台，要是自摸就＊３，相当于五一的算法。
