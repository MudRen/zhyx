...
string ask_bf();
varargs void drool(string msg, object who);
int do_open();
int do_close();
string *rnd_say = ({
	"我忽然好想去滑冰.",
...
void create()
{
	set_name("童话", ({"snowbaby"}));
	set("nickname", HIM "风起、雨飘、花落、无痕" NOR);
	set("long", HIC "\n她长得极美，尤其是那双眼睛，媚得直可以勾人魂魄。\n"+
	"其容色晶莹如玉，映照于红红烛光之下，娇美不可方物。\n"+
	"回眸嫣然一笑，雅如仙子下凡。眼波流转之间，\n"+
	"便把一个少年的心灵，推向梦中极美的境界。\n" NOR);
	set("title", HIC "石城大姐大" NOR);
	set("gender", "女性");
	set("age", 21);
	set("rank_info/respect", "姑娘");
        set("can_reply_channel",1);
	set("max_qi", 10000);
	set("max_jing", 10000);
	set("neili", 5000);
	set("max_neili", 5000);
	...
	set("chat_msg", ({
		...
		(: command("laughproud") :),
		"童话忽然大声说：打倒臭泥潭，打倒鱼儿！\n",
		"童话无聊地绕着手绢，叹道: 唉，一个人，真没意思。\n",
		"童话伸出白玉雕成一般的小手，托着下巴说: 谁想和我说说话?\n",
		"童话斩钉截铁地说: 嗯，谁要敢欺负我，我就宰了他！\n",
		"童话忽然莞尔一笑道: 鱼儿待会儿又要我杀谁呢？\n",
		"童话打了个呵欠，左手轻轻掩住樱桃小口，柔声道：我困了。\n",
		(: drool :),
		(: drool :),
		(: command("reclaim") :),
		(: command("reclaim") :),
...
int do_open()
{
	this_object()->set("can_reply_channel",1);
	return notify_fail("童话的聊天频道被打开了！\n");
}

int do_close()
{
	this_object()->set("can_reply_channel",0);
	return notify_fail("童话的聊天频道被关闭了！\n");
}
...
void relay_emote(object ob, string verb)
{
	...
	{
	command ("chat 鱼儿快来啊，"+ ob->query("name") + "想要非礼我！\n") ;
	command("chat 哎哟，谁想得到这" + RANK_D->query_rude(ob)+"，竟也是一个好色之徒 。");
	}
	else
	{
	message_vision("\n童话对$N笑道：“你我同为女儿身，再怎么喜欢也不要这样啊。”\n", ob);
	command("giggle");
   ...
   switch(random(50))
   {
    ...
    case 8:
    command("cry");
   command ("chat 鱼儿快来啊，"+ ob->query("name") + "想要羞辱我！\n") ;
...
varargs void drool(string msg, object arg)
{
	mapping mem;
	string *ob,who;
	...
	case 7:	this_object()->set("killer","童话");break;
	...
	if( (strsrch(msg, "snowbaby") >= 0)||(strsrch(msg, "童话") >= 0))
	{	
	...
	{	
		...
		case 3:	command("chat* nod");	break;
		case 4:command("chat* cry"); break;
		case 5:command("chat "+who+",你真要走了就别再回来！\n"); break;
		case 6:	command("chat* sigh");	break;
		case 7:	command("chat "+who+",再玩一会儿好不好?\n");	break;
		case 8:	command("chat "+msg);	break;
		case 9:command("chat 噢，"+who+"是天底下最好的人了!\n"); break;
		case 10:command("chat* byebye "+arg->query("id")); break;
		case 11:command("chat* foot "+arg->query("id")); break;
		case 12:command("chat* wave "+arg->query("id")); break;
		}
		return;
	}
	if( (strsrch(msg, "kiss") >= 0)|| (strsrch(msg, "love") >= 0)|| (strsrch(msg, "吻") >= 0)
		|| (strsrch(msg, "mo") >= 0)|| (strsrch(msg, "抱") >= 0)|| (strsrch(msg, "hug") >= 0)
		|| (strsrch(msg, "爱") >= 0)|| (strsrch(msg, "makelove") >=0)|| (strsrch(msg, "开房间") >=0))
		{
		...
		if( sscanf(msg, "%*smakelove%s", msg) == 2 ) msg = "makelove" + msg;
		if( sscanf(msg, "%*s吻%s", msg) == 2 ) msg = "吻" + msg;
		if( sscanf(msg, "%*s抱%s", msg) == 2 ) msg = "抱" + msg;
		if( sscanf(msg, "%*s摸", msg) == 2 ) msg = "摸"+ msg;
		if( sscanf(msg, "%*s开房间", msg) == 2 ) msg = "开房间"+ msg;
		if( sscanf(msg, "%*s爱", msg) == 2 ) msg = "爱"+ msg;
		...
		case 3:command("chat* faint "+arg->query("id")); break;
		case 4:command("chat* cry"); break;
		case 5:command("chat 不要这样子嘛！\n"); break;
		case 6:command("chat 非礼啊非礼！"+who+"这小流氓又来啦！\n"); break;
		case 7:command ("chat 鱼儿快来啊，"+ who + "想要羞辱我！\n") ;
		case 8:	command("chat* heng "+arg->query("id")); break;
		case 9:	command("chat* angry");	break;
		case 10:command("chat* shrug");	break;
		case 11:command("chat 你对女孩子总是这样吗？\n"); break;
		case 12:command("chat 大哥你怎么这样？\n"); break;
		case 13:command("chat* kick "+arg->query("id")); break;
		case 14:command("chat* wuwu "+arg->query("id"));	break;
		case 15:command("chat 嗯，"+who+"是天底下最好的人了！\n"); break;
	    case 16:command("chat 鱼儿快来啊，"+ who + "欺负我！\n") ; break;
		case 17:command("chat* poke2 "+arg->query("id")); break;
		case 18:command("chat* black "+arg->query("id")); break;
		}
		return;
	}
	...
	{	
		...
		case 3:	command("chat* angry");	break;
		...
		case 9:	command("chat* die "+arg->query("id")); break;
		case 10:command("chat* faint "+arg->query("id")); break;
		case 11:command("chat* slogan"); break;
		case 12:command("chat* heng1 "+arg->query("id")); break;
		case 13:command("chat 鱼儿快来啊，"+ who + "又想要羞辱我！\n") ; break;
		case 14:command("chat* hammer "+arg->query("id")); break;
		case 15:command("chat 谁敢骂我是猪，我就剁了谁！\n"); break;
		case 16:command("chat* kok "+arg->query("id")); break;
		case 17:command("chat "+who+"你要死啊？小心我踢死你哦！\n"); break;
		}
	}
	...
	{	
		...
		case 3:	command("chat* fight2 "+arg->query("id"));	break;
		case 4:	command("chat 你真的认为“" + msg + "”？\n"); break;
		case 5:	command("chat 我觉得你有点儿缠杂不清。\n"); break;
		case 6: command("chat 呸！就凭你这个小王八蛋？\n"); break;
		case 7:	command("chat 你怎么知道“" + msg + "”？\n"); break;
		case 8:	command("chat "+ who+"滚出来!我要和你决斗！有胆子你就放马过来！\n"); break;
		case 9:	command("chat* die "+arg->query("id")); break;
		case 10:command("chat* cry"); break;
		case 11:command("chat* "+who+"王八蛋给我滚出去！呜呜。。。。\n"); break;
		case 12:command("chat 非礼啊非礼！"+who+"是个小变态！\n"); break;
		}
		return;
	}
	...
	{	
		...
		case 3:	command("chat* humble"); break;
		case 4:	command("chat 你真的认为“" + msg + "”？\n"); break;
		case 5: command("chat 你这是不是有点儿。。。？嘻嘻嘻。。。\n");break;
		case 6:    command("chat* zhen1 "+arg->query("id"));break;
		case 7: command("chat 嗯,"+who+"是一个好孩子.\n"); break;
		case 8:command("chat "+who+",你不会是想讨好我吧?\n"); break;
		case 9:command("chat* same");break;
		case 10:command("chat* praise "+arg->query("id"));break;
		case 11:command("chat* shy");break;
		case 12:command("chat* cm "+arg->query("id"));break;
		case 13:command("chat* lean "+arg->query("id"));break;
		}
		return;
        }
	}
	if(  (strsrch(msg, "kill") >= 0)||(strsrch(msg, "杀") >= 0)||(strsrch(msg, "pk") >= 0))
	{	
		if(random(5)<4)
        {
		switch(random(35)) {
		...
		case 1:	command("chat 你是在说我吗？\n"); break;
		case 2:	command("chat "+who+",没想到你也这么。。。\n"); break;
		case 3:	command("chat* alarm"); break;
		case 4:	command("chat 你真的认为“" + msg + "”？\n"); break;
		case 5: command("chat 你这是不是有点儿。。。？嘿嘿！\n");break;
		case 6:    command("chat* oh? "+arg->query("id"));break;
		case 7: command("chat 嗯,"+who+"是一个小王八蛋！\n"); break;
		case 8:command("chat "+who+",你不会是想去见白无常吧?\n"); break;
		case 9:command("chat 就凭你个小王八蛋，也敢跟我来说什么打打杀杀！");break;
		case 10:command("chat* pk "+arg->query("id"));break;
		case 11:command("chat* heng");break;
		case 12:command("chat* nomatch "+arg->query("id"));break;
		case 13:command("chat* dong "+arg->query("id"));break;
		case 14:command("chat* hide "+arg->query("id"));break;
		case 15:command("chat* afraid");break;
		case 16:command("chat* pai1");break;
		case 17:command("chat* papa");break;
		case 18:command("chat* dong snowbaby");break;
		case 19:command("chat* escape");break;
		case 20:command("chat* slap "+arg->query("id"));break;
		case 21:command("chat 我还怕了你不成？出来，我们单挑！\n");break;
		case 22:command("chat* knife2");break;
		case 23:command("chat* poke2 "+arg->query("id"));break;
		case 24:command("chat* papa1 "+arg->query("id"));break;
		case 25:command("chat* fight1 "+arg->query("id"));break;
		case 26:command("chat* dong "+arg->query("id"));break;
		case 27:command("chat* black "+arg->query("id"));break;
		case 28:command("chat 阎王在哪里啊，阎王在哪里？阎王就在"+who+"这小王八蛋家里头。\n");break;
		}
		return;
        }
	}
	switch(random(90)) {
		...
		case 12:	command("chat* sigh");	break;
		case 13:	command("chat* fly");	break;
		case 14:	command("chat 你。。。 。哼！不理你！\n");	break;
		case 15:    command("chat* qifu");break;
		case 16:    command("chat 嘻嘻，我说啊，你这又是何苦呢？\n");break;
		case 17:    command("chat* zzz"); break;
		case 18:    command("chat* trip "+arg->query("id"));break;
		case 19:    command("chat 哼，我都懒得理你。\n");break;
		...
		case 24:	command("chat* sing "+arg->query("id")); break;
		case 25:	command("chat 居然有人骂我是猪!\n");
		 	command("chat* cry"); break;
        case 26:	command ("chat 鱼儿快来啊，"+ who + "又来欺侮我！\n") ; break;
      	case 27:	command ("chat 我简直受不了了!\n") ; break;
        case 28:	command ("chat 我恨不得"+who+"去死!\n") ; break;
        case 29:	command ("chat 就你老和我过不去!\n") ; break;
        case 30:	command ("chat 请不要打扰我好不好?\n") ; break;
   		case 31:	command ("chat* wuwu") ; break;
		case 32:	command("chat* jiejie "+arg->query("id")); break;
		case 33:    command("chat 嗯，谁要敢欺负我，我就宰了他！\n");break;
		case 34:    command("chat* yi");break;
		case 35:    command("chat* fever"); break;
		case 36:    command("chat* hammer "+arg->query("id"));break;
		case 37:    command("chat 哼，我都懒得理你。\n");break;
		...
		case 42: command("chat* xixi "+arg->query("id")); break;
		case 43: command("chat 烦着哪，别理我!\n"); break;
		case 44: command("chat 哼！\n"); break;
		case 45: command("chat* slap "+arg->query("id")); break;
		case 46: command("chat 哼，谁理你啊!\n");break;
		case 47: command("chat 嗯,"+who+"是一个好孩子.\n"); break;
		case 48:command("chat* ?"); break;
		...
		case 54:command("chat* praise "+arg->query("id"));break;
		case 55:command("chat 不要这样子嘛！\n"); break;
		case 56:command("chat* same");break;
		case 57:command("chat* 你怎么知道“" + msg + "”？\n"); break;
		case 58:command("chat* zou "+arg->query("id"));break;
		case 59:command("chat* black "+arg->query("id"));break;
		case 60:command("chat 为什么呢?\n");break;
		case 61:command("chat 你是在说我吗？\n"); break;
		case 62:command("chat 无聊!\n");break;
		case 63:command("chat* shy");break;
		case 64:command("chat 我就是一个自恋狂，不服气？\n");break;
		case 65:command("chat* di "+arg->query("id")); break;
		case 66:command("chat "+who+"，说你胖你就喘，说你漂亮你就不洗脸！\n");break;
		case 67:command("chat* cm "+arg->query("id")); break;
		case 68:command("chat* jinya "+arg->query("id")); break;
		case 69:command("chat* fever "+arg->query("id")); break;
		case 70:command("chat "+who+"也可以算一个美人胚子，只可惜眼睛小了点！\n");break;
		case 71:command("chat* hema "+arg->query("id")); break;
		case 72:command("chat* club");break;
		case 73:command("chat 不要以为我怕了你，看你是小王八蛋我才不杀你！\n");break;
		case 74:command("chat "+who+"，你也就是江湖中一个跑龙套的。\n");break;
		case 75:command("chat* pai1");break;
		default:
	    if (random(10)<3) command("chat* angry"); 
	    else if(random(10)<3) command("chat* laugh");
	    break; 
		}
	return;
	} 
	...
	{	
	   if(random(5)<3)
		...
		case 2:command("chat* xixi"); break;
		case 3:command("chat* yi"); break;
		case 4:command("chat* cry"); break;
		case 5:command("chat* diuren "+arg->query("id")); break;
		case 6:command("chat* sigh");	break;
		case 7:command("chat* shrug");	break;
		case 8:command("chat* pat "+arg->query("id")); break;
		case 9:command("chat* protect1 "+arg->query("id"));break;
		case 10:command("chat* bianzui "+arg->query("id"));break;
		case 11:command("chat* nono! "+arg->query("id"));break;
		case 12:command("chat* cry3 "+arg->query("id"));break;
		case 13:command("chat* corpse "+arg->query("id"));break;
		case 14:command("chat* doubt "+arg->query("id"));break;
		case 15:command("chat* careful");break;
		case 16:command("chat "+who+"，你哭的样子好迷人耶！\n");break;
		case 17:command("chat 一天到晚哭哭啼啼干什么呀，真是的！\n");break;
		case 18:command("chat* cryxiao snowbaby");break;
		}
		return;
        }
	}
	...
	{	
		...
		case 9:	command("chat* shrug");	break;
		case 10:command("chat* foot "+arg->query("id"));break;
		case 11:command("chat* bye2 "+arg->query("id"));break;
		}
		return;
	}
	if( (strsrch(msg, "猪") >= 0)||(strsrch(msg, "shit") >= 0)||(strsrch(msg, "pig") >= 0))
	{	
		...
		case 11:	command("chat* laugh"); break;
		case 12:	command("chat* shake");	break;
		case 13:	command("chat* applaud"); break;
		case 14:	command("chat* oh?"); break;
		case 15:	command("chat* zhen1 "+arg->query("id"));break;
		case 16:	command("chat* poke2 "+arg->query("id"));break;
		case 17:	command("chat* papa1 "+arg->query("id"));break;
		}
		return ;
        }
	} 
		...
		case 12: command("chat* consider"); break;
		case 13: command("chat 与其求人,不如求己!\n"); break;
		case 14: command("chat* gua "+arg->query("id"));break;
		case 15: command("chat* konglong "+arg->query("id"));break;
		case 16: command("chat 恩，"+who+"是泥巴里第一大笨蛋！\n");break;
		case 17: command("chat* dong "+arg->query("id"));break;
		}
 	return;
               }
	...
	||	(strsrch(msg, "you") >= 0)) {
		...
		case 12:	command("chat* sigh");	break;
		case 13:	command("chat* laugh"); break;
		case 14:	command("chat who?\n"); break;
		case 15:	command("chat 这小王八蛋又在那里叽叽歪歪，真讨厌！\n");break;
		case 16:	command("chat 似乎以前也听人这么讲过。。。\n");break;
		case 17:	command("chat* shutup");break;
		}
	return;
	}
	if( (strsrch(msg, "笑") >= 0))
	{	
		...
		case 3:	command("chat "+who+"笑起来真丑。\n"); 
			if(random(10)<2) command("chat* 童话嘻嘻地笑了几声.\n");break;
		case 4:	command("chat* giggle");break;
		case 5:	command("chat* nogig");break;
		case 6: command("chat* pat "+arg->query("id")); break;
		case 7:	command("chat* laugh"); break;
		case 8: command("chat* same" );break;
		...
		case 11:drool(); break;
		case 12:drool(); break;
		case 13:command("chat* xixi"); break;
		case 14:command("chat* careful"); break;
		case 15:command("chat 我对此表示怀疑.\n"); break;
		case 16:command("chat* oh?");	break;
		case 17:command("chat* zhen1"); break;
		...
		switch(random(250)) {
			...
			case 2: command("chat* heng"); break;
			case 3: command("chat* lazy"); break;
			case 4: command("chat* faint"); break;
			case 5: command("chat  哼！\n"); break;
			case 6: command("chat* yi"); break;
			case 7: command("chat* face"); break;
			...
			case 11: command("chat* faint "+arg->query("id")); break;
			...
			case 18: command("chat 这世道，还是各人自扫门前雪，休管他人瓦上霜.\n");break;
			case 19: command("chat* bow");break;	
			case 20: command("chat 我知道了，" + msg+"\n"); break;
			case 21:command("chat* consider"); break;
			...
			case 25: command("chat* 一个人，真没意思。\n"); break;
			case 26: command("chat* doubt"); break;
			case 27: command("chat* consider "+arg->query("id")); break;
			case 28: command("chat  我看未必.\n"); break;
			case 29: command("chat 好奇怪，有这等事情？\n"); break;
			case 30: command("chat  老师来啦，"+who+"你还不赶快去？\n"); break;
			case 31: command("chat "+who+",你今天怎么又逃课?\n"); break;
			case 32: command("chat* thank"); break;
			case 33: command("chat* laugh");break;
			case 34: command("chat "+who+"说得好!\n"); break;
			case 35: command("chat* gua "+arg->query("id")); break;
			case 36: command("chat "+who+",没想到你也会这样.\n"); break;
			case 37: command("chat "+who+",你无聊不无聊啊?"); break;
			case 38:  command("chat* poke2 "+arg->query("id"));break;
			case 39:command("chat* di "+arg->query("id")); break;
			case 40:command("chat 嗯 ... " + who + "说得好！\n"); break;
			case 41:command("chat* shrug");	break;
			case 42:command("chat 嗯，"+who+"是天底下最好的人了！\n"); break;
			case 43:command("chat* shake "+arg->query("id"));	break;
			case 44:command("chat 我对此表示怀疑.\n"); break;
			case 45: command("chat 我觉得最佳表情奖应该授予"+who+".\n"); break;
			case 46: command("chat* cm "+arg->query("id")); break;
			case 47: command("chat* heihei "+arg->query("id")); break;
			...
			case 55: command("chat* slap "+arg->query("id")); break;
			case 56:command("chat* alarm "+arg->query("id")); break; 
			case 57:command("chat 不要这样嘛!\n"); break;
			case 58:command("chat 要死啊你!"+who+"!\n"); break; 
			case 59:command("chat 打倒臭泥潭，打倒鱼儿！\n"); break;
			case 60:command("chat* yi"); break;
			case 61:command("chat 我们热爱绝地末日，绝地末日万岁！\n");break;
			case 62:command("chat 既然大家如此不要脸，我就把大家的脸都糊房子去了。\n");break;
			case 63:command("chat* nod");	break;
			case 64:command("chat* laugh"); break;
			case 65: command("chat 与其求人,不如求己!\n"); break;
			case 66:command("chat* disapp");break;
			case 67:command("chat 无聊!好无聊!\n");break;
			case 68:command("chat 是吗?\n"); break;
		        case 69:command("chat 变态！\n"); break;
			case 70:command("chat 我就是一个自恋狂，谁不服气？\n");break;
			case 71:command("chat 为什么？\n");break;
			case 72:command("chat 你烦不烦啊？\n");break;
			case 73:command("chat* konglong "+arg->query("id"));break;
			case 74:command("chat* hema "+arg->query("id"));break;
			case 75:command("chat* papa1 "+arg->query("id"));break;
			case 76:command("chat* qia "+arg->query("id"));break;
			case 77:command("chat* heihei");break;
			case 78:command("chat* pai1");break;
			case 79:command("chat* pk2 "+arg->query("id"));break;
			case 80:command("chat* black "+arg->query("id"));break;
			case 90:command("chat* dong");break;
			case 91:command("chat* zou "+arg->query("id"));break;
			case 92:command("chat* kok "+arg->query("id"));break;
			case 93:command("chat* cook "+arg->query("id"));break;
			case 94:command("chat* praise "+arg->query("id"));break;
			case 95:command("chat* admire "+arg->query("id"));break;
			default:
	    if (random(10)<1) command("chat* ?");
		else if (random(10)<1) command("chat* xixi");
	    else if (random(10)<1) command("chat* lazy");
				break;
		}
	}
}
...
int do_tell ( string arg )
{
   ...
   if( sscanf(arg, "liling %s", dir)==1 ) {
		...
		{
		command("tell "+ob->query("id")+" 你尽管自个儿爱吧。我就对鱼儿还有点儿兴趣。");
...
int accept_object(object me, object ob)
{
	...
	if (ob->query("money_id") && ob->value() >= 1000000 )
	{
		...
		{
		say("\n童话摇摇头，笑道：不好吧，杀人的事鱼儿不会同意的。\n");
		me->set_temp("fee_paid2",1);
		tell_object(me, "\n童话嫣然一笑，说：不过，我可以开个小玩笑。\n");
	...
	{
	message_vision("\n$N猛的扬起头，用惊喜的眼光看着$n，说：\n"
	+"咦，这不是鱼儿送我的戒指吗，是他要你带来的么? 真谢谢你啊...\n",this_object(),me);
	}
	if ((string)ob->query("name")==HIW "水晶鞋" NOR) 
	{
	message_vision("\n$N猛的扬起头，用惊喜的眼光看着$n，说：\n"
	+"咦，这不是鱼儿送我的吗水晶鞋吗，是他要你带来的么? 真谢谢你啊...\n",this_object(),me);
	}
	...
	{
	message_vision("\n$N缓缓地摇了摇头，柔声说道:不是鱼儿送我的,我不要.\n"
	+"说罢垂下头来，咬了咬牙,幽幽嗔道：死鱼儿，臭鱼儿，为什么还不回来呢?\n",this_object());
	return 0;
	}
}
...
int do_name(string target)
{
	...
	if(me->query("waiting_target"))
		return notify_fail("童话：我现在正忙着哪，您呆会儿再来吧！\n");
	if( !this_player()->query_temp("fee_paid") ) 
		return notify_fail("童话打了个呵欠，左手轻轻掩住樱桃小口，柔声道：\n"+
		"不好吧，帮你杀了人我又有什么好处呢？\n");
	if( !target || target==" ")
		return notify_fail("童话打了个呵欠，左手轻轻掩住樱桃小口，柔声道：\n"
		+"请告诉我那人的名字，我可没功夫跟你闲聊。\n");
	...
	if( !dest )
		return notify_fail("童话做了个鬼脸：不太好啊，好像他不在喔。\n");

	if (dest->is_ghost())
		return notify_fail("童话说：你黑，你太黑，你太太黑，你太太太黑！他已经死啦！\n");
	me->set("waiting_target", dest->query("id"));
	me->set("target_name", target);
	me->delete("chat_chance");
	me->delete("chat_msg");
	message("vision",
	HIY "\n童话正要离开，忽又回首道：你别走，就在这儿静侯佳音吧。\n" NOR,
	environment(), me );
	me->set("pursuer", 1);
...
int do_kill(object me, object dest)
{
	...
	{
		message_vision(HIR "$N对$n说道：鱼儿要我来杀你，对不起啊。。。\n"
			+RANK_D->query_rude(dest)+"，你就认命吧！\n" NOR, me, dest);
			if(wizardp(dest))		
	                { 
			dest->set("immortal",0);
...
int do_back(object me)
{
	...
	message("vision", "童话走了进来，幽幽道：唉，鱼儿真不乖，害得我又杀了人。\n", 
		environment(), me );
	me->set_leader(0);
	set("chat_chance", 3);
	set("chat_msg", ({
