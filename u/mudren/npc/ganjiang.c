...
int do_help(string arg)
{
	...
	message_vision(HIR "$N" HIR "长啸一声，双手搭上炉边，刹那间，炉火沸腾，"
        ...
        ob->add("item/point", ob->query("item/point") * me->query_skill("force") / 1000);
	me->delete_temp("item/can_help");
...
void waiting(object me, object ob, object moye)
{
	...
	if (environment(me) != environment())
	{
        ...
        if (! in_input(me))
	{
		message_vision("$N催$n道：快点，快点，别磨蹭。\n",
                               this_object(), me);
	}
...
int do_wieldmsg(string arg)
{
	...
	ob = query_temp("item/making");
        ...
        if (!arg || strsrch(arg,"$N")==-1 || strsrch(arg,"$n")==-1)
	{
