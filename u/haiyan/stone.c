...
int do_jump(string arg)
{
        ...
        {       
                ...
                for(int i = 0; i < sizeof(inv); i++)
                        if( userp(inv[i]) )
                        {
                                inv[i]->receive_wound("jing", inv[i]->query("max_jing")*9/10);
                                inv[i]->receive_wound("qi", inv[i]->query("max_qi")*9/10);
                        }
                        else    destruct(inv[i]);
...
void dropoff(object me)
{
        ...
        for(int i = 0; i < sizeof(inv); i++)
                if( userp(inv[i]) )
                {
                        inv[i]->receive_wound("jing", inv[i]->query("max_jing"));
                        inv[i]->receive_wound("qi", inv[i]->query("max_qi"));
                }
                else    destruct(inv[i]);
