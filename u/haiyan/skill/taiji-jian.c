...
int valid_learn(object me)
{
        ...
        if ((int)me->query_skill("taiji-quan", 1) < 100)
                return notify_fail("你的太极拳水平太低，不能领悟太极剑法。\n");

        if ((int)me->query_skill("sword", 1) < 100)
                return notify_fail("你的基本剑法火候太浅。\n");

        if ((int)me->query_skill("sword", 1) < (int)me->query_skill("taiji-jian", 1))
                return notify_fail("你的基本剑法水平有限，无法领会更高深的太极剑法。\n");

        if ((int)me->query_skill("taiji-quan", 1) < (int)me->query_skill("taiji-jian", 1))
                return notify_fail("你的太极拳水平有限，无法领会更高深的太极剑法。\n");
