...
#define POTIAN      "/u/sanben/special/lonely/potian.h"
#define CHIJIAN     "/u/sanben/special/lonely/chijian.h"
...
{             
	...
	msg = HIY "你专心的研读墙上的古怪图形。\n" NOR;
   switch (random(5)) 
	{
	    case 0:
		if ((int)me->query_skill("force", 1) < 229)
		{
			me->improve_skill("force", 1 + random(me->query("int")));
			msg += "你对着墙琢磨了一回儿，似乎对内功有点心得。\n";
			me->set_temp("stone_learned", 1);
		}
                break;

	    case 1:
		if ((int)me->query_skill("dodge", 1) < 229)
		{
			me->improve_skill("dodge", 1 + random(me->query("int")));
			msg += "你对着墙琢磨了一回儿，似乎对轻功有点心得。\n";
			me->set_temp("stone_learned", 1);
		}
                break;
		...
		if ((int)me->query_skill("strike", 1) < 229)
		{
			...
			msg += "你对着墙琢磨了一回儿，似乎对掌法有点心得。\n";
			me->set_temp("stone_learned", 1);
		}
                break;

	    case 3:
		if ((int)me->query_skill("parry", 1) < 229)
		{
			me->improve_skill("parry", 1 + random(me->query("int")));
			msg += "你对着墙琢磨了一回儿，似乎对招架有点心得。\n";
			me->set_temp("stone_learned", 1);
		}
                break;

	    case 4:
		if ( (int)me->query_skill("sword", 1) < 229)
		{
			me->improve_skill("sword", 1 + random(me->query("int")));
			msg += "你对着墙琢磨了一回儿，似乎对剑法有点心得。\n";
			me->set_temp("stone_learned", 1);
		}
	}

	if (! me->query_temp("stone_learned"))
		msg += HIY "你对着墙琢磨了一回儿，发现上面所说的已经太"
                       "过浅显了。\n" NOR;
...
int do_linghui( string arg )
{
   ...
   if (! query("light") )
    ...
    if ( arg == "傲气中华" 
        && me->query("can_perform/china-aojue/aoqi"))
          return notify_fail("这招你不是已经会了吗？\n");
    ...
    if (random(15) == 1 && arg == "傲气中华")
    {
        me->add("china-aojue/aoqi/count", 1);

        if (me->query("china-aojue/aoqi/count") >= 20)
        {
            write(HIM "猛然间，你胸中豁然贯通，再无疑虑。\n" NOR);
            write(HIC "你终于通晓了绝学「傲气中华」。\n" NOR);

            me->set("can_perform/china-aojue/aoqi", 1);
            me->delete("china-aojue/aoqi/count");
