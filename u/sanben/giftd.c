...
int do_place()
{
   ...
   message( "gift", BLINK HIY"【天降礼物】" NOR HIG
            "天神眼见人间疾苦，在" + dir_name + "撒下了一些神物，大家何不群起而寻之！\n"NOR, users() );
   ...
   for(i = 0;i < 20;i++)
   {
      ...
      if (  place )
      {   
          if ( i <= 1 ) gift = new("/u/dgdg/fuben/obj/magic_item/donghuangzhong");
          else 
          if ( i <= 2 ) gift = new("/u/dgdg/moonblade(auto)");
          else 
          if ( i <= 3 ) gift = new("/u/dgdg/obj/qixing_sword");
          else 
          if ( i > 2 && i <= 7 ) gift = new("/u/dgdg/obj/moon cake");
          else 
             gift = new("/u/dgdg/obj/giftpack.c");            
          gift->move(place);
          tell_room(place, HIC"突然只见天神从云端里探出头来，撒下一样东西。\n"NOR);
      }
   }
   return 1; 
