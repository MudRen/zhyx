...
#include <ansi.h>
#define   TASK_OBJ_PATH  "/u/sanben/task/obj/"       //task物件目录
...
{ 
        ...
        string msg, *msg_arr, dir_name, dir_total, str;
        mapping dir;
    ...
    foreach(dir_name in keys(dir))
        ...
        write(ESC+"[r"+TOBOT(80));
     
        write(SAVEC);   
        
        for (x = 1; x < sizeof(msg_arr) + 5; x++)
        write(SETDISPLAY(x, 0) + DELLINE); 
       
        write(SETDISPLAY(1, 0) + HIY"+++++++++++++++++++++++++++++"
               "+++++++++++++++++++++++++"NOR);  
        str = WHT "乾坤宝镜显示" NOR +
              ob->name() + NOR WHT"现在所在地方的描述是:\n\n" NOR;    

        write(SETDISPLAY(2, 0) + str); 
        
         for (x = 3; x < sizeof(msg_arr) + 3; x++)
         {             
             write(SETDISPLAY(x, 0) + msg_arr[x-3] );
             if (x == sizeof(msg_arr) + 2 )
            { 
              x = x + 1;
              write(SETDISPLAY(x, 0) + "这个地方的出口有" + HIG + dir_total + NOR);
              x = x + 1;
              write(SETDISPLAY(x, 0) + HIY"++++++++++++++++++++"
                    "++++++++++++++++++++++++++++++++++"NOR); 
              x = x + 1;
              write(SETDISPLAY(x, 0) + DELLINE);
             }
