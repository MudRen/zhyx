...
#define ZIXU          "/u/sanben/xquest/npc/zixu"      //task发布宣布者
#define TASK_CARRIER  "/u/sanben/task/npc/task_carrier" //task 携带者
#define TASK_OBJECT   "/u/sanben/task/obj/"            //task物件目录
#define MIRROR        "/u/sanben/task/mirror.c"        //宝镜放置的路径
        ...
        if (! task)
           {
                ...
                if ( npc->query("max_qi") < 500 ) npc->set("max_qi",500);
                if ( npc->query("max_jing") < 500 ) npc->set("max_jing",500);
...
int do_return(object ob, object me, string arg)
{
 ...
 /*      if ( count >= 12 ) count = 12;
    ...
    if ( count > 20 ) pot = 600 + random(100);
    else
    if ( count > 10 ) pot = 500 + random(100);
    else
    pot = 400 + random(100);
  ...
  //  me->add("experience", tihui);
    ...
    tell_object( me, HIY"这是你这一轮完成的第" +  HIR +
                  ...
                  HIG"点潜能。\n"NOR
                  HIW"外加奖励十两白银，以及" + chinese_number(gx) +
                  "点门派贡献。\n"NOR);
...
string set_item(object me)
{
        ...
        if (me->query("mirror_count") == 500)
        {
         ...
         if ( ! gift ) gift = "/clone/fam/gift/dex3";
         item = new(gift);
         item->move(me);
