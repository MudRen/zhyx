...
{    
     ...
     {		
      ...
      if ( dz && ! objectp(present(dz->query("id"), environment(me))) ) 
      {
          message_vision(HIW"$N对$n道：你不是抓来了盗贼吗，怎么不带上堂来？\n"NOR,ob, me);
          return 0;
      }   
    ...
    {      
        message_vision(HIR"$N"HIR"对$n大怒道：你连个小小的窃贼都抓不住，"
                       "还有何面目来向本府复命！\n"NOR, this_object(), me);
        return 0;
    }*/
...
{                
       ...
       if ( me->query("changanjob_count") % 100 == 0 ||
            me->query("changanjob_count") % 50 == 0  )  //每完成一百个任务给予item奖励
...
{                
        ...
        string un, gift;
        object ob;
        gift = "clone/fam/pill/zhengqi"; //50个任务奖励正气果
