...
int perform(object me, object target)
{
        ...
        if (userp(me) && ! me->query("can_perform/yeqiu-quan/jiandao"))
        ...
        if (objectp(me->query_temp("weapon")))
                return notify_fail("你必须空手才能使用"	JIAN "！\n");
                
        if ((int)me->query_skill("yeqiu-quan", 1) < 250)
                return notify_fail("你的野球拳还不够娴熟，不能使用" JIAN "。\n");
        ...
        if (me->query_skill_prepared("unarmed") != "yeqiu-quan"
           ...
           && me->query_skill_prepared("cuff") != "yeqiu-quan")
                return notify_fail("你没有准备野球拳，无法使用" JIAN "。\n");
                                
        if ((int)me->query("neili", 1) < 1000)
                return notify_fail("你现在内力太弱，不能使用" JIAN "。\n");
	...
	if (me->query("qi") > me->query("max_qi")/2 && target->query("qi") > target->query("max_qi")/2)
        {
		...
		msg += HIG "两人同时报数：“一！二！三！”\n\n"
			"$N"HIG"右手握拳往前一伸，赫然是一个" JIAN HIG"。\n" NOR;

		switch(random(3))
		{
		case 1:
			msg += HIG"$n"HIG"右手一伸，竟然出了一个" SHI HIG"。\n" NOR;
			msg += HIC "$N黯然道，我输了！没想到我居然输了！\n"
				HIC "$N只觉得胸口一疼，竟然呕吐出了一大口鲜血！\n" NOR;
			me->add("neili", -1000);
			me->receive_damage("qi", me->query("max_qi")/2-10);
			break;
		case 2:
			msg += HIG"$n"HIG"右手一伸，出了一个" BU HIG"。\n" NOR;
			msg += HIC "$n惨然道，“我竟然输了！我竟然也会输！”\n"
				HIC "$n刚说完，口中竟然不断吐出鲜血！\n" NOR;
			me->add("neili", -1000);
			target->receive_damage("qi", target->query("max_qi")/2-10);
			break;
		default :
			msg += HIG"$n"HIG"右手一伸，竟也出了一个" JIAN HIG"。\n" NOR;
