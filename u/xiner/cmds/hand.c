...
int main(object me, string arg)
{
	...
	if (! arg)
        ...
        if (arg == "none" || arg == "nothing")
        {
                ...
                if ((string)ob->query("id") == "qi jing")
                        return notify_fail("这东西可不能放进包裹里面！\n");
		//四象神功产生的qi jing不能放回身上
	...
	if (! objectp(ob = present(arg, me)))
        ...
        {
                if ((string)old->query("id") == "qi jing")
                        return notify_fail("你手中凝结着气劲，不能拿其他东西！\n");
		//四象神功产生的qi jing不能放回身上
	 ...
	 if ( ob && ob->query("race") == "野兽")
		return notify_fail("野兽你也能抓手里吗?别开玩笑了。\n");
