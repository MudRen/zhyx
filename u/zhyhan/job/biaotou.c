...
int do_move(string arg)
{
        ...
        if (!arg) {
                ...
                command("whisper "+me->query("id")+" 你要我赶到哪儿去？");
                return notify_fail("\n");
                }

        if( !mapp(exit = env->query("exits")) || undefinedp(exit[arg]) ) {
                command("?");
                command("whisper "+me->query("id")+" 这个方向没有路呃。");
                return notify_fail("\n");
                }
        ...
        if (me->is_busy()) {
                command("whisper "+me->query("id")+" 你自己都忙不过来，还催我干什么？");
                return notify_fail("\n");
                }

        if (ob->is_fighting()) {
                command("whisper "+me->query("id")+" 请你先把我的对手料理了吧！\n");
                return notify_fail("\n");
                }
...
int ask_start()
{
        ...
        if (ob->query_temp("started")) {
                command("whisper "+me->query("id")+" 不是已经在路上了吗？");
                return 1;
                }
        ...
        if (me->query("quest/yunbiao") ==1) {
                ...
                command("whisper "+me->query("id")+" 好吧，我们出发！");
                return 1;
                }
        
        if (ob->query_temp("owner/id") != me->query("id")) {
                command("slap " + me->query("id"));
                command("whisper "+me->query("id")+" 又没有让你护镖，来瞎闹什么？");
                return 1;
                }
        call_out("leave", 600);
...
int ask_arrived()
{
        ...
        if (ob->is_busy()) {
                command("whisper "+me->query("id")+" 我这儿正忙着呢！");
                return notify_fail("\n");
                }
        ...
        if (ob->query_temp("fuzhou") ==1){
                 if (file_name(environment(ob)) !="/d/fuzhou/biaojuzt") {
                         command("whisper "+me->query("id")+" 你想半途而废？");
                       return 1;
                       }
        ...
        message_vision(HIW "$N被奖励了：\n"
                ...
                + HIR + chinese_number(gx) + "点贡献，\n"
                + HIC + chinese_number(th) + "点体会，\n"                
