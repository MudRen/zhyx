...
void create()
{
        ...
        set_name( HIC "中华英雄动议投票专用箱" NOR, ({ "vote box", "box", "ballot" }) );
        set("no_get", 1);
        set("no_sac", 1);
        set("no_clean_up", 1 );
        set("unit", "座");
...
string short()
{
        ...
        if( !query("voting") )
                return HIC "投票专用箱 " NOR "(vote box) [ " HIR "没有投票活动 " NOR "]";
        if( !arrayp( ballots ) || !sizeof( ballots ) )
                return HIC "投票专用箱 " NOR "(vote box) [ " HIG "投票中" NOR ", " HIM "没有任何选票 " NOR "]";
        if( this_player() ) {
                if( query("voting_start_time") > time() ) {
                        return sprintf("%s " NOR "[ %s, " HIM "投票尚未开始 " NOR "]",
                        HIC "投票专用箱 " NOR "(vote box)",
...
string long()
{
        ...
        long = HIY "  [ 中华英雄动议投票专用箱 ]" NOR "\n";
        long += HIG"\n  投票说明：[ 要投票请按 toupiao 选项编号 ]  [要取消投票请按 cancel 选项编号 ]\n"NOR;
        if( !query("voting") )
                return long + HIR "\n  目前没有任何投票活动" NOR "\n";
        ...
        long += HIR" <投票简述> "NOR WHT+query("desc") + "\n"NOR;
        long += HIR"────────────────────────────────────────\n"NOR;

        long += HIW " <提名状况> " HIY "[提名开始时间]： " NOR + BJTIME_CMD->chinese_time(1,ctime( query("voting_setup_time")) ) + "\n";
        long += "            " HIY "[提名截止时间]： " NOR + BJTIME_CMD->chinese_time(1,ctime( query("final_nominate_time")) ) + "\n";
        long += "            " + ( time() < query("final_nominate_time") ? HIG "提名中" NOR "\n\n" : HIR "提名截止" NOR "\n\n" );

        long += HIW " <投票状态> " NOR;
        long += HIY "[投票开始时间]： " NOR + BJTIME_CMD->chinese_time(1,ctime( query("voting_start_time")) )+ "\n";
        long += "            " + ( time() < query("voting_start_time") ? HIR "投票尚未开始" NOR "\n" : HIG "投票开始" NOR "\n" );
        if( query("max_voter_option") == 0 )
                long += HIC "            不限票数\n";
        else
                long += sprintf( HIC "            每人最高投票数: " HIR "%s\n\n"NOR, query("max_voter_option")
                        ? sprintf("%d", query("max_voter_option")) : "不限制投票数" );
...
int do_setup( string arg )
{
        ...
        if( !wizardp( this_player() ) )
                return notify_fail("只有巫师有权力更动投票动议的设定.\n");
...
────────────────────────────────────────
stat                    查看选票情况
setup reset             重新设定所有资料(慎重使用)
setup desc    内容      投票活动的叙述
setup id      记号      投票活动的识别代码 ( 每次投票都要更改 )
setup final   数字      最后提名时限, 以天数计
setup start   数字      几天之后开始投票, 以天数计. ( 0 则马上开始 )
setup max_op  数字      每人可投的票数
setup voting  数字      是否开始投票. 0 则结束, 非零值则开始 ( 请最后设定 )
nominate      内容      设定一个投票选项
────────────────────────────────────────
LONG
                );
                return 1;
...
int do_nominate( string arg )
{
        ...
        message_vision(HIG"$N在"HIC"投票专用箱"HIG"内丢入一张纸, 上面写着:\n" HIW + arg + NOR "\n", this_player() );
...
int do_vote( string arg )
{
        ...
        if(this_player()->query("combat_exp") < 1000000)
          return notify_fail("你武学过低，没有资格投票！\n");

        if(!userp(this_player()))
          return notify_fail("只有玩家可以投票!!!\n");
        options = query("options");
        ballots = query("ballots");
        player_ballots = this_player()->query("vote/ballots");
        ...
        if(query("vote_player/"+ppl_id) < query("max_voter_option"))
          ...
          return notify_fail(WHT"你投超过了，一个人只能投"HIR+
CHINESE_D->chinese_number(query("max_voter_option"))+NOR WHT"次！\n");
        ...
        message_vision(HIG"$N把一张选票扔进"HIC"中华英雄动议投票专用箱"HIG"里。" NOR "\n", this_player() );
...
int do_cancel( string arg )
{
        ...
        message_vision(WHT"$N把一张选票丢进"HIC"投票专用箱"NOR WHT"里消掉了。" NOR "\n", this_player() );
...
int do_statistics( string arg )
{
    ...
    if(!wizardp(me)) return 0;
        ...
        if( !arg ) {
                ...
                write( stat );
                message_vision(WHT"$N打开"HIC"投票专用箱"NOR WHT"偷看选票。" NOR "\n", this_player() );
