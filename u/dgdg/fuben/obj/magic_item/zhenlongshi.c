...
void create()
{
        ...
        {
                set("unit", "块");
                set("long", NOR + YEL "这是一块泛着七彩光芒的石头，传说此石乃黄帝的护身符。\n"
                                      "只要带在身上死亡后不会丢失经验，潜能以及技能。\n"
                                      "此神器下线后不丢失。这块石头好像可以吃(chi)下去耶。\n" NOR);
...
int do_eat(string arg)
{
        ...
        if (arg && id(arg))
        {
          ...
          if (random(5)!=1) 
          	{
                log_file("static/using", sprintf("%s(%s) use 真龙石 at %s.\n",
           ...
           else 
           	{
           		...
           		tell_object(me, HIW "\n你啃啊啃啊，把大板牙都啃嘣了，可是没等把石头吃下去，却把这块天下奇石啃坏了！哟，不好了！\n" NOR);
           		destruct(this_object());  
           		return 1;
              }
