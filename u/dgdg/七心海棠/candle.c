...
// 通过look candle可以获取蜡烛还能使用的次数。
        if (ob->query("yaowang/candle/using") == 1)
            descrp += YEL "已经快用完啦，看上去只能再用一次。\n" NOR;
        else if (ob->query("yaowang/candle/using") == 2)
                descrp += YEL "大约有一寸多长，看上去并不是用来照明的。\n" NOR;
        else if (ob->query("yaowang/candle/using") == 3)
                descrp += YEL "长度大约有一寸半多点，看来也用不了几次。\n" NOR;
        else if (ob->query("yaowang/candle/using") == 4)
                descrp += YEL "大约有二寸多长，恰好能攥在手心里。\n" NOR;
        else
            descrp += YEL "不过看上去好象出了问题，快点找巫师吧。\n" NOR;
        return descrp;
}
...
int do_dian(string arg)
{
        ...
        if (arg == "candle" || arg == "haitang candle")
        {
            if (! me->query_temp("qu_candle"))
                return notify_fail("你手里没有蜡烛。\n");
            message_vision(HIY "$N小心翼翼地点燃了手中的蜡烛，烛光黯淡，火焰随风摇曳，忽明忽暗。\n\n" NOR, me);
            me->start_busy(1 + random(2));
...
// 点燃蜡烛，可以导致所有在场的人中毒致死，内力精深者运气好的可以不死，但会暂时失明。
            if (random((int)ob[i]->query("neili")) < 200000)
            {
                message_vision(HIB "$N" HIB "双膝渐渐弯曲，身子软了下来，脸上似笑非笑，神情极是诡异。\n" NOR, ob[i]);
            ...
            else
            {
                ...
                message_vision(HIB "$N" HIB "双手在空中乱抓乱扑，两眼无神，神情极是恐怖。\n" NOR, ob[i]);
...
// 判断蜡烛是否还能再用，如果无法再用则destruct。
        if (me->query("yaowang/candle/using") < 1) 
        {
            ...
            message_vision(HIY "$N手中的蜡烛点到了尽头，只见忽地一亮，火焰吐红，一声轻响过后，蜡烛熄灭了。\n\n" NOR, me);
            destruct(this_object());
        }
        else message_vision(HIY "一阵风吹了过来，$N手中的蜡烛，烛火一闪，登时熄了。\n\n" NOR, me);
