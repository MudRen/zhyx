...
void create()
{
        ...
        {
            set("base_unit", "棵");
            set("long", HIY "花瓣紧贴枝干而生，花枝如铁，花瓣上有七个小小的黄点。\n" NOR);
            set("no_sell", 1);        
...
int do_eat(string arg)
{
        ...
        message_vision(HIB "$N" HIB "感觉周围事物逐渐离自己而去，双膝渐渐弯曲，身子软了下来。\n" NOR, me);
...
//              ob->clear_condition(); // 叶子解天下奇毒，此处使用了Doing设计的标记nopoison解毒。
                ...
                message_vision(HIR "一阵剧痛过后，" + ob->query("name") + "发现体内的束缚已经全部散去了。\n" NOR, me);
            ...
            if (what == "hua" || what == "花") 
            {
                ...
                message_vision(CYN + ob->query("name") + "只觉得周身穴位一阵热麻，全身气血充盈，内力精力充沛之极。\n"NOR, me);
...
//              ob->clear_condition(); // 解天下奇毒，此处使用了Doing设计的标记nopoison解毒。
                ...
                message_vision(HIR "一阵轻微的疼痛过后，" + ob->query("name") + "发现体内的束缚已经全部散去了。\n" NOR, me);
                me->start_busy(2 + random(3));
                destruct(this_object());
