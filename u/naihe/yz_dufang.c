...
// ----------------- defines -----------------------
#define     DUFANG_LOG_FILE                 "yz_dufang"     // log dir 下。
#define     LOG_ON          1               // 开启时则进行 log_file() 操作。主要是记录 mewiz 操作记录。
#define     DUFANG_SAVE_FILE    DATA_DIR +  "ya_dufang"
#define     SAVE_ON         1               // 开启时则进行 save_object() 操作。
                                            // 本文件数据记录位置
#define     SYSTEM_WIZ_ID   "$naihe$yuchang$"       // 允许使用 <mewiz reward 数字> 指令给系统加入奖金。
...
string query_now_long2()
{
    ...
    if( game_info[ "last_opened_msg" ] )
        msg += BBLU + HIW"  上局：" + delcolor(game_info[ "last_opened_msg" ]) + "  \n"NOR;
    else
        msg += BBLU + HIW"  上局：                                                            \n"NOR;
...
string query_now_long()
{
    ...
    msg ="这里是扬州城中新开的小赌坊，别看它规模不大，却常常有许多千金富豪
...
private int do_mewiz( string arg )
{
    ...
    if( arg == "stat" )
    {
        ...
        msg = sprintf( HIG"【公告】%s(%s)将扬州城%s%s了，现在：%s。\n"NOR,
            me->query("name"), getuid(me), query("short"), (BetOn ? "开放" : "关闭"),
            (BetOn ? "正常营业" : "停止营业")
        );
        shout( msg );
    ...
    if( stringp(arg) && sscanf( arg, "reward %d %s", add, because ) == 2 )
    {
        ...
        msg = sprintf( HIG"【公告】%s(%s)给扬州城%s注入了 %d 两黄金作额外奖励资金，原因：%s\n"NOR,
