...
int ask_reborn(); 
string ask_2015NewYearGift();
string ask_2009NationalGift();
string ask_20131111();
int ask_biwu(); 
int ask_lonely(); 
...
{
    set_name(HIG"小侠"NOR, ({"kasumi_boss"}));
    ...
    set("max_qi", 80000);
        ...
        set("inquiry", ([ 
                    "转世" : (: ask_reborn :),                   
                    "新年礼物" : (: ask_2015NewYearGift :),      
					"华山论剑" : (: ask_biwu :), 
		//			"国庆礼物" : (: ask_2009NationalGift :), 
					"独孤九剑" : (: ask_lonely :), 
		//			"光棍节礼物" : (: ask_20131111 :), 
                ]));      
...
string ask_20131111()
{
        ...
        if (me->query("gift2013/1111"))
        {
			me->add("potential",-10000);
			tell_object(me, HIG"由于你的贪心，被扣除一万潜能。\n"NOR);
            return "要了还想要啊？真贪心！！\n";
        }      
                
        me->add("combat_exp",100000);
        me->add("potential",100000);
		tell_object(me, HIG"恭喜你增加了十万经验和十万潜能。\n"NOR);
        me->set("gift2013/1111", 1);
...
int ask_biwu()
{
    ...
    if (me->query("age")<18)
    	{
    		command("say 你未满18岁，无民事行为能力不能参赛！");
        return 1;
    	}
	...
	if (living(this_object()) && present("yingxiong tie", me))
	{
		room = find_object("/adm/npc/huashanjueding");
	        if (! room) room = load_object("/adm/npc/huashanjueding");
	        me->move(room);
	} else
	{
                command("grin");
