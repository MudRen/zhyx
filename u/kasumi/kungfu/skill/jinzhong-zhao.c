...
int valid_learn(object me)
{
        ...
        if ((string)me->query("gender") != "男性") 
        	return notify_fail("金刚罩乃是至刚至阳之功，你如何可以修习？\n");       
        ...
        if ((int)me->query_skill("force", 1) < (int)me->query_skill("jinzhong-zhao", 1))
                return notify_fail("你的基本内功水平有限，无法领会更高深的金刚罩。\n");
...
mixed valid_damage(object ob, object me, int damage, object weapon)
{
        ...
        if ((int)me->query_skill("jinzhong-zhao", 1) < 150 ||
            (int)me->query("neili") < 400 || !me->query_temp("jinzhongzhao"))
                return;
        ...
        ap = ob->query_skill("force") * 15 + ob->query("max_neili") + mp * 12;
        dp = me->query_skill("force") * 16 + me->query("max_neili");
