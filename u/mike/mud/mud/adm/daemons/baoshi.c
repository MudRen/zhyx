...
void choose_baosi()
{
        ...
        if(rd==0) 
        {
                 newob = new("/u/kan/lvbaoshi");
                 str = "绿";
        }
        if(rd==1) 
        {
                 newob = new("/u/kan/hongbaoshi");
                 str = "红";
        }
        if(rd==2)
        { 
                 newob = new("/u/kan/huangbaoshi");
                 str = "黄";
        }
                
        if(random_place(dirs[j],newob)== 1 )
        message("channel:chat",HIG"【"+HIR"小道消息"+HIG"】"+HIW"江湖传闻：近日盛传"+dirname[j]+"附近有"+str+"光浮现，似有人私藏宝藏!\n"+NOR,users());
