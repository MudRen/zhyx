...
int do_list()
{
	...
	foreach (string skill in e_special)
        ...
        msg += HIC"请使用选择(choose)指令，选择你需要的一个special(格式为：choose 中文名)。\n"NOR;
        write(msg);
...
int check_user()
{
	object me = this_player();
	//没打开前世记忆呢
	if (!me->query("reborned") && !wizardp(me))
	{
...
int do_decide(string i)
{
	...
	if ( i != "1" && i != "2" && i != "3")
		...
		foreach (string skill in keys(me->query_temp("rb_special" + i + "/")))
       	...
       	command("say 你已经彻底转世完毕了，你可以使用wake指令离开这里了。");
