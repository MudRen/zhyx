...
#include <config.h>
#include <getconfig.h>
//#include "/u/xgchen/mail_center/mail.h" // 包含读取邮件信息的函数
...
private void check_question(string answer,object ob);//安全判断函数声明
...
//#if 0
        if (iplimit > 10) //一个ip提高为10个id by yuchang
        {
...
//#endif
        if (iplimit > 1)
        	printf(HIG"您所在的地址已有 " HIY "%d" NOR+HIG " 位玩家在线上。\n"NOR,
		       iplimit-1);
	printf(HIR"目前共有 " HIC "%d" NOR+HIR " 位巫师、"
	       HIC "%d" NOR+HIR " 位玩家在线上，以及 "
	       CYN "%d" NOR+HIR " 位使"
	       "用者尝试连线中。\n"NOR, wiz_cnt, ppl_cnt, login_cnt);

	write(HIR"您的英文名字(新玩家可以选择一喜欢的名字)："NOR);
	input_to("get_id", ob);
}
...
*/
	...
	if (arg == "guest")
		...
		if (ob->restore())
                {
		if (ob->query("login_safety"))
		{
			write(HIY "您使用了安全判断验证\n"
			"您的验证问题是：" + HIC + ob->query("login/question") + "\n" + NOR
			 HIY"请输入您的验证答案:" NOR);
			input_to("check_question", 1, ob);
...
// 安全判断函数
private void check_question(string answer,object ob)
{
	if (ob->query("login/answer") != answer )
	{
		  write(HIR "您输入的答案不匹配，请重新登陆！"NOR);
		  destruct(ob);
		  return;
	}
		write(HIM"验证通过，请输入密码："NOR);
		input_to("get_passwd", 1, ob);
		return;
}
...
private void confirm_password(string pass, object ob)
{
	...
	if (crypt(pass, old_pass) != old_pass)
        ...
        write(WHT "\n请您选择（输入数字" HIC "1" NOR + WHT "—" HIC "5"
              NOR + WHT "）在「" HIR "中华英雄" NOR + WHT "」中您想要"
              "扮演的角色类型：
┏━━━━━┳━━━━━┳━━━━━┳━━━━━┳━━━━━┓
...
private void get_gender(string gender, object ob)
{
	...
	ob->set("login_safety",0);//初始设置安全为空
	ob->set("registered", 0);
...
varargs void enter_world(object ob, object user, int silent)
{
	...
	if (!ob->query("login_safety"))
	write(HIR + HBWHT "提醒：您目前没有设置安全登陆验证，请使用“safety”指令进行设置。\n" NOR);
	...
	if( user->query("age") < 16 )
 	...
 	//天子给宝甲
   /*    if(user->query("special_skill/emperor") && (user->query("age") > 15) )
        {
        	user->set("can_summon/qingtian","/data/item/"+user->query("id")+"/qintian");
        }           */
...
//user->set("born",1);
	...
	if (! silent)
        ...
        login_ob->restore();
      if ( wizardp(user) ) login_ob->delete("old_ps");
                ...
                } else
                {
                        login_ob->add("login_times", 1);
	                     write("\n你上次光临" + LOCAL_MUD_NAME() + "是 " + HIG +
                              "/cmds/usr/bjtime"->chinese_time(8,ctime((login_ob->query("last_on")))) + NOR + " 从 " + HIR +
...
*/
        "/cmds/usr/msg"->main(this_player(), "read all");
