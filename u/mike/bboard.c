...
void done_post(object me, mapping note, int n, string text)
{
        ...
        if(sizeof(text)>=3000){//这个数字是测试出来的。呵呵 by xgchen
...
int do_post(string arg, int n)
{
  ...
  if((time()-(int)this_player()->query("postover")) < 60 && !wizardp(this_player()))
            return notify_fail("你已经在不久留言了，请稍等1分钟再尝试。\n");
        ...
        note["title"] = arg;
          if(sizeof(arg)>=40){//这个数字是测试出来的。呵呵by xgchen
...
"----------------------------------------------------------------------
 标 题  %-62s
 作 者  %-19s   ╭══════════════════╮
 篇 数  %-11d           ║"HIR"请文明使用，否则"HIY"你"HIR"或"HIY"帖子"HIR"可能会被删除"NOR"║
 时 间  %-12s          ╰══════════════════╯
----------------------------------------------------------------------\n\n",
